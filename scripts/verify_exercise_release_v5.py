...
RELEASE = ROOT / "book/always_on/exercises/release-v5.json"
...
def check_student_code(source: str) -> None:
    """Inspect executable syntax; discussion of solutions in prose is not an import."""
    ...
    for node in ast.walk(tree):
        ...
        require(not any("solutions" in name.split(".") for name in modules), "solution import")
        if isinstance(node, ast.Call):
            ...
            if callee in {
                ...
                require(
                    not any("solutions" in re.split(r"[/\\.]", value) for value in strings),
                    "solution import",
                )
...
def verify(release_path: Path = RELEASE) -> None:
        ...
        for row in release[group]:
            ...
            if group == "studentFiles":
                ...
                if path.endswith(".ipynb"):
                    for cell in json.loads(target.read_text())["cells"]:
    ...
    required_runtime = {
        ...
        "scripts/build_exercise_release_v5.py",
        "scripts/verify_exercise_release_v5.py",
