...
covers the contributor surface:
  ...
  - mypy passes for the currently enforced public runtime modules
...
def main() -> int:
    ...
    rc, out = run("uv", "run", "ruff", "format", "--check", *quality_paths)
    if not check("ruff format", rc == 0, "" if rc == 0 else f"exit {rc}"):
        issues += 1
    ...
    # 6. mypy on public runtime modules that are currently clean
    rc, out = run("uv", "run", "mypy", *PUBLIC_RUNTIME_PATHS)
    if not check("mypy public runtime", rc == 0, "" if rc == 0 else f"exit {rc}"):
        issues += 1

    # 7. pytest collection (no imports failed, no syntax errors)
    ...
    # 10. example scenarios importable
