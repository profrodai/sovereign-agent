#!/usr/bin/env python3
"""V2: recognize paired practical Jupytext sources; retain all prior curriculum checks.
...
def check_no_frontmatter() -> list[str]:
    ...
    site's collection schema. V2 also admits the paired ninety-minute practical sources.
    """
    problems: list[str] = []
    for markdown_file in sorted(BOOK.rglob("*.md")):
        ...
        is_practical = bool(
            re.fullmatch(
                r"always_on/practicals/ninety-minute-v1/ch(?:0[1-9]|1[0-6])/"
                r"(?:student|instructor)/unit-[ab]-v1\.md",
                relative.as_posix(),
            )
            and markdown_file.with_suffix(".ipynb").is_file()
            and frontmatter
            and re.findall(r"^([^ \t\n][^:\n]*):", frontmatter.group(), re.MULTILINE) == ["jupyter"]
        )
        is_jupytext = bool(
            frontmatter
        ...
        if frontmatter and not ((is_exercise or is_practical) and is_jupytext):
            problems.append(
