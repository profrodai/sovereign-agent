#!/usr/bin/env python3
"""verify_setup.py — deterministic release-gate diagnostics for sovereign-agent.

The default gate checks a clean checkout without requiring credentials or
...
configuration and perform a real completion round-trip.
  ...
  - Mypy clean for the currently enforced public runtime modules
  - With ``--live``: .env, API key, and a tiny LLM completion
...
def check_ruff() -> int:
    try:
        ...
        if result.returncode == 0:
            ok("ruff lint clean across package, tests, chapters, examples, and gates")
            return 0
...
def check_ruff_format() -> int:
    try:
        result = subprocess.run(
            ["uv", "run", "ruff", "format", "--check", *QUALITY_PATHS],
            cwd=REPO_ROOT,
            capture_output=True,
            text=True,
            timeout=60,
        ...
        warn(f"ruff format check could not run: {e}")
        return 0
...
def check_mypy() -> int:
    try:
        result = subprocess.run(
            ["uv", "run", "mypy", *PUBLIC_RUNTIME_PATHS],
            cwd=REPO_ROOT,
            capture_output=True,
            text=True,
            timeout=60,
        ...
        if result.returncode == 0:
            ok("mypy clean for enforced public runtime modules")
            return 0
        fail("mypy public runtime check failed")
        hint("Run: make typecheck")
        return 1
    except Exception as e:  # noqa: BLE001
        warn(f"mypy check could not run: {e}")
        return 0
...
async def run_checks(*, live: bool = False) -> int:
    print(f"\n{BOLD}{BLUE}{'=' * 58}{RESET}")
    print(f"{BOLD}{BLUE}  sovereign-agent — pre-flight verification{RESET}")
    print(f"{BOLD}{BLUE}{'=' * 58}{RESET}")
    info("Runs deterministic release checks; live LLM probing is opt-in.")
    ...
    dotenv_contents = load_dotenv(REPO_ROOT / ".env")
    ...
    if live:
        ...
        section("LLM API key")
        key_err, api_key, _key_var = check_llm_api_key(merged_env)
        errors += key_err
    ...
    if live:
        ...
        if api_key:
            ...
            hint("Fix the .env issues above, then re-run: make verify-live")
            errors += 1
    else:
        section("LLM endpoint")
        info("Live probe skipped (opt in with: make verify-live)")

    # ── Summary ─────────────────────────────────────────────────────────
    print(f"\n{BOLD}{'=' * 58}{RESET}")
    if errors == 0:
        print(f"{GREEN}{BOLD}✓  All requested checks passed.{RESET}")
        if not live:
            print(f"  {MAGENTA}Optional live probe:{RESET} {GREEN}make verify-live{RESET}")
        print()
    else:
...
def main() -> None:
    ...
    parser.add_argument(
        "--live",
        action="store_true",
        help="require LLM credentials and perform a real completion round-trip",
    )
    args = parser.parse_args()
    sys.exit(asyncio.run(run_checks(live=args.live)))
