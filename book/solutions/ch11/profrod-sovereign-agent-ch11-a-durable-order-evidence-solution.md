...
jupyter:
  ...
  course:
    ...
    planned_minutes: 90
    resource_id: profrod-sovereign-agent-ch11-a-durable-order-evidence-solution
...
with tempfile.TemporaryDirectory(prefix="ch09-transition-hidden-") as directory:
    ...
    try:
        ...
        assert (unknown["reserved_cents"], unknown["spent_cents"]) == (1100, 0)
        assert final["status"] == replay["status"] == "REJECTED"
        assert (replay["reserved_cents"], replay["spent_cents"]) == (0, 0)
        try:
