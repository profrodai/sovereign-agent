...
jupyter:
  ...
  course:
    ...
    planned_minutes: 90
    resource_id: profrod-sovereign-agent-ch09-b-scheduling-repair-transfer-solution
...
def transfer_check(due, interval, now, capacity):
    if now < due or capacity == 0:
        return [0, due, 0]
    skipped = int((now - due) // interval)
    return [1, due + (skipped + 1) * interval, skipped]
```
...
Advancing by one interval after a long absence leaves the job due. Advancing when capacity is zero loses the deferred occurrence. Both timing and admission outcome must determine the stored next time.
...
# Instructor holdout appended to a submitted Chapter 7 Unit B.
...
try:
    ...
    outcome = task.transfer(ROOT / "book/always_on/exercises/ch07/holdouts/runtime-transfer-v1.py")
