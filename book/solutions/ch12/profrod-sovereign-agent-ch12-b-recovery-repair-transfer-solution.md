...
jupyter:
  ...
  course:
    ...
    planned_minutes: 90
    resource_id: profrod-sovereign-agent-ch12-b-recovery-repair-transfer-solution
...
def transfer_check(current, claim, now):
    return (
        not current["paused"]
        and all(current[k] == claim[k] for k in ("epoch", "generation", "work", "worker"))
        and claim["lease_until"] > now
    )
```
...
A matching worker name is insufficient. Each authority field and the lease boundary must remain current. Positive and one-field-stale cases prevent one check from hiding the absence of another.
...
INSTRUCTOR_TRANSFER_CASES = [
    (
        "paused",
        [
            {"paused": True, "epoch": "new", "generation": 4, "work": "opening", "worker": "w1"},
            {"epoch": "new", "generation": 4, "work": "opening", "worker": "w1", "lease_until": 20},
            10,
        ],
        False,
    ...
    (
        ...
        [
            {"paused": False, "epoch": "new", "generation": 4, "work": "opening", "worker": "w1"},
            {"epoch": "new", "generation": 4, "work": "opening", "worker": "w2", "lease_until": 20},
            10,
        ],
        False,
    ),
...
try:
    ...
    task.close()
print("HOLDOUT_RESULT=" + json.dumps({"unit": "ch12-b", "status": "PASSED"}, sort_keys=True))
```
