...
jupyter:
  ...
  course:
    ...
    planned_minutes: 90
    resource_id: profrod-sovereign-agent-ch17-a-bounded-delegation-solution
...
def transfer_check(guests, price_cents, portions):
    if any(type(v) is not int or v <= 0 for v in (guests, price_cents, portions)):
        raise ValueError("positive integers required")
    tubs = (guests + portions - 1) // portions
    return [tubs, tubs * price_cents]
```
...
# Instructor holdout appended to a submitted Chapter 14 Unit A.
...
try:
    ...
    outcome = task.transfer(ROOT / "book/always_on/exercises/ch14/holdouts/runtime-transfer-v1.py")
    assert outcome["status"] == "PASS", outcome
finally:
    task.close()
print("HOLDOUT_RESULT=" + json.dumps({"unit": "ch17-a", "status": "PASSED"}, sort_keys=True))
```
