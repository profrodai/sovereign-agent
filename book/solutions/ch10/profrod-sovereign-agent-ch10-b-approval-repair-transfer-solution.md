...
jupyter:
  ...
  course:
    ...
    lesson_id: approval
    planned_minutes: 90
    resource_id: profrod-sovereign-agent-ch10-b-approval-repair-transfer-solution
...
def repair_fragment():
    return 'if budget["spent_cents"] + budget["reserved_cents"] + addition > min('
```
...
def transfer_check(spent, reserved, addition, installed, supplied):
    ...
    return spent + reserved + addition <= min(installed, supplied)
```
...
# Instructor holdout appended to a submitted Chapter 8 Unit B.
