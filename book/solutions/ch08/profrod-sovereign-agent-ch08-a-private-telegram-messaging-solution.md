...
jupyter:
  ...
  course:
    ...
    lesson_id: messaging
    planned_minutes: 90
    resource_id: profrod-sovereign-agent-ch08-a-private-telegram-messaging-solution
...
def transfer_check(updates, seen, allowed):
    ...
    for u in updates:
        ...
        eligible = (
            type(u["sender"]) is int
            and type(u["chat"]) is int
            and u["sender"] == u["chat"]
            and u["sender"] in allowed
            and u["kind"] == "private"
            and isinstance(u["text"], str)
            and 1 <= len(u["text"]) <= 100
        )
        if eligible and identity not in known:
            result.append(list(identity))
            known.add(identity)
    return result
```
...
INSTRUCTOR_TRANSFER_CASES = [
    ...
    (
        "empty content",
        [
            [{"account": "a", "id": 3, "sender": 7, "chat": 7, "kind": "private", "text": ""}],
            [],
            [7],
        ],
        [],
...
# Instructor holdout appended to a submitted Chapter 6 Unit A.
...
try:
    ...
    outcome = task.transfer(ROOT / "book/always_on/exercises/ch06/holdouts/runtime-transfer-v1.py")
