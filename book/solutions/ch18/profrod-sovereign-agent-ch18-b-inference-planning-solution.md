...
jupyter:
  ...
  course:
    ...
    lesson_id: operation
    planned_minutes: 90
    resource_id: profrod-sovereign-agent-ch18-b-inference-planning-solution
...
This is the worked edition of Chapter 18, Unit B. It contains:
...
def kv_cache_bytes(config, tokens, bytes_per_value=2):
    ...
    if config["attention_heads"] % config["kv_heads"] or tokens < 0:
        raise ValueError("KV heads must divide attention heads, and tokens must be at least 0")
    head_dim = config.get("head_dim", config["hidden"] // config["attention_heads"])
    return 2 * config["layers"] * config["kv_heads"] * head_dim * bytes_per_value * tokens
```
...
def percentile(values, q):
    ...
    return ordered[math.ceil(q * len(ordered) / 100) - 1]
```
...
**"Every head stores keys and values."** Only KV heads do. The colleague's estimate is six times too large for `qwen2.5:1.5b`. Their prediction of its long-context decode rate misses by about 11%, while the correct one lands within 6%. A subtle case: for `qwen3:0.6b`, the colleague's two errors cancel. Using all 16 heads doubles the cache, and using the quotient 64 instead of the declared 128 halves it again. So the colleague's per-token figure happens to be right. A formula that passes one case for the wrong reason is why the visible cases include several configurations.

**"A formula that matches the fitted runs is validated."** Fitting two parameters to two runs always matches those two runs. The evidence is the third kind of measurement, decode after a long prompt, which the fit never saw.

**"p99 of thirty requests is meaningful."** It is the maximum, a single observation. Tail percentiles need many requests, and a shared service's tail is set by queueing, which this idle-machine run did not exercise.

The cases below add a request tail, and the recorded latencies.

```python tags=["instructor-check"]
INSTRUCTOR_TRANSFER_CASES = [
    ("one slow request sets p99", [[0.38] * 29 + [2.0], 99], 2.0),
    ("but not p90", [[0.38] * 29 + [2.0], 90], 0.38),
]
instructor_observations = run_transfer(percentile, INSTRUCTOR_TRANSFER_CASES)
assert TRANSFER_PASSED and all(row["passed"] for row in instructor_observations)
print(
    "recorded requests: p50",
    percentile(REQUEST_SECONDS, 50),
    "p90",
    percentile(REQUEST_SECONDS, 90),
    "p99",
    percentile(REQUEST_SECONDS, 99),
)
```

```python tags=["instructor-check", "core-holdout"]
assert kv_cache_bytes(CONFIGS["qwen2.5:1.5b"], 32_768) == 939_524_096
assert kv_cache_bytes(CONFIGS["qwen3:0.6b"], 40_960) == 4_697_620_480
assert (
    kv_cache_bytes({"layers": 1, "attention_heads": 8, "kv_heads": 8, "hidden": 64}, 3)
    == 2 * 1 * 8 * 8 * 2 * 3
)
assert connected is not None and connected["fits"] and connected["negative_control_rejected"]
assert percentile(REQUEST_SECONDS, 100) == max(REQUEST_SECONDS)
assert percentile(REQUEST_SECONDS, 1) == min(REQUEST_SECONDS)
print("HOLDOUT_RESULT=" + json.dumps({"status": "PASSED", "unit": "ch18-b"}, sort_keys=True))
```
