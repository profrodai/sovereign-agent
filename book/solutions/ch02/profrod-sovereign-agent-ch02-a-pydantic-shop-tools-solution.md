...
jupyter:
  ...
  course:
    ...
    planned_minutes: 90
    resource_id: profrod-sovereign-agent-ch02-a-pydantic-shop-tools-solution
...
def transfer_check(row):
    return max(0, row["target"] - row["on_hand"] + row.get("reserved", 0))
```
...
# Instructor holdout appended to a submitted Chapter 2 Unit A.
...
try:
    ...
    task.close()
print("HOLDOUT_RESULT=" + json.dumps({"unit": "ch02-a", "status": "PASSED"}, sort_keys=True))
```
