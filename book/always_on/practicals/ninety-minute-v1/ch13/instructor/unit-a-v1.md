...
def transfer_check(before, current, results, required):
    return (
        bool(required)
        and before == current
        and all(name in results and results[name] is True for name in required)
    )
```
...
# Instructor holdout appended to a submitted Chapter 13 Unit A.
...
try:
    ...
    task.close()
print("HOLDOUT_RESULT=" + json.dumps({"unit": "ch13-a", "status": "PASSED"}, sort_keys=True))
```
