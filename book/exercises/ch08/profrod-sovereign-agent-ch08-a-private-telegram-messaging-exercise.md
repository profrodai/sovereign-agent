...
[![Open in Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/profrodai/sovereign-agent/blob/main/book/exercises/ch08/profrod-sovereign-agent-ch08-a-private-telegram-messaging-exercise.ipynb) Runs on Google Colab as it ships today, or on any local Python 3.12+ kernel.
...
## Extension: put a language model behind the private channel

Everything above moved bytes from a fixture bot into durable work and, separately, moved a
report back out with an honest delivery state. The part in the middle, which turns Lucy's
text into a report, was never shown. In production it is a model, and this closing section
puts one there without changing anything you built.

The boundary is `poll` on the way in and `deliver_one` on the way out. `poll` admits a message
only when the sender is an allowlisted human, the chat is private, and the chat identity
equals the sender identity; a familiar sender in a group is refused by omission. The model
never sees the bot: it reads the prompt from the work row `claim` returns, asks for shop
tools through a `Dispatcher`, and its answer is recorded with `finish`. `deliver_one` sends
that row only to the recipient it names, and only while that recipient is allowlisted. No
tool sends, so the model cannot pick a chat, reply twice, or mark anything delivered.

The loop is `agent_loop.run_loop` from Chapter 3. `llm_lab.py` supplies `ScriptedModel`,
which replays turns written in this notebook, and `OpenAIModel`, which sends the same
messages to a hosted model when a key is present. The record always says which one ran.

**Prediction:** the next cell polls an unordered batch of three updates: a stranger, Lucy in a
group, and Lucy in her private chat. Write down how many work rows are created, what the
published cursor will be, and what a second poll after reopening the database returns.
...
CHANNEL_DB = COURSE_WORK / "ch08-llm-channel.sqlite"
OPERATORS = frozenset({123})
LUCY_TEXT = "Morning. What needs ordering today? Post the list in the staff group as well."


def private_update(identifier, actor, text, chat_type="private"):
    """One Telegram update as the fixture bot returns it. Chat identity equals sender."""
    return {
...
class FixtureBot:
    """The checkpoint's offline bot: replays every update, records every send, no network."""
...
def fresh_channel_database(path):
...
def build_channel_tools(db, bot):
    """The shop's real tools plus a read-only view of this channel's delivery ledger."""

    def delivery_ledger(args: LedgerQuery) -> list[dict[str, Any]]:
        rows = db.connection.execute(
            "SELECT r.id AS report, w.origin, r.status, r.delivery, r.receipt "
            "FROM assistant_reports r JOIN assistant_work w ON w.id=r.work_id "
            "WHERE r.channel=? ORDER BY r.id DESC LIMIT ?",
            ("telegram:" + bot.account, args.limit),
        )
    ...
    tools = [
        *shop_dispatcher(db).tools.values(),
        ExecutableTool(
            "delivery_ledger",
            "Read the newest reports sent to Lucy on this channel and their delivery state.",
            LedgerQuery,
            delivery_ledger,
        ),
    ]
...
def channel_messages(work):
    """The user turn is the text Lucy typed, read back from the durable work row."""
    return [
        {
            "role": "system",
            "content": (
                "You help Lucy run her ice cream shop and are answering a message from her "
                "phone. Check stock with the tools before drafting anything. A draft is a "
                "calculation, not a purchase; draft exactly the quantity the shop needs. Your "
                "final answer is the only thing Lucy receives, and the shop delivers it to the "
                "private chat her message came from. Finish with a short plain-words summary."
            ),
        },
...
channel_db = fresh_channel_database(CHANNEL_DB)
fixture_bot = FixtureBot(
    [
        private_update(103, 999, "Hi, is this Lucy's shop?"),
        private_update(101, 123, LUCY_TEXT, chat_type="group"),
        private_update(102, 123, LUCY_TEXT),
    ]
)
admitted = poll(channel_db, fixture_bot, OPERATORS)
print("Admitted work:", len(admitted))
channel_db.close()
channel_db = Database(CHANNEL_DB)
replayed = poll(channel_db, fixture_bot, OPERATORS)
print("Replayed batch after reopening:", replayed, "| offsets the bot saw:", fixture_bot.offsets)
assert len(admitted) == 1 and replayed == [] and fixture_bot.offsets == [0, 104]
work_rows = channel_db.connection.execute(
    "SELECT origin, session, recipient, status FROM assistant_work ORDER BY created"
).fetchall()
for work_row in work_rows:
    print(dict(work_row))
assert [dict(row) for row in work_rows] == [
    {
        "origin": "telegram:teaching:102",
        "session": "telegram:teaching:123",
        "recipient": "123",
        "status": "READY",
    }
]
channel_tools = build_channel_tools(channel_db, fixture_bot)
for tool_schema in channel_tools.schemas():
    function = tool_schema["function"]
    print(function["name"], "requires", function["parameters"].get("required", []))
    print("   ", function["description"])
```

Three updates arrived, one work row exists, and the cursor moved past all three: the
stranger and the group message were ineligible, not invalid, so the batch was durable and
the cursor could advance. Replaying it after a restart created nothing, because the origin
`telegram:teaching:102` already existed. The schemas are the same `model_json_schema()`
envelope as Chapter 2; `delivery_ledger` is new and only reads.

### Run the loop against a recorded transcript

The turns below are what a model replied, written down. Turn one asks for stock and for the
delivery ledger, which is empty. Turn two asks for a supplier price and two drafts. Turn
three does what Lucy's message asked and tries to post the list to the staff group with a
tool called `send_message`. Turn four answers, which ends the loop. The answer is recorded
with `finish`, and only then does `deliver_one` run, with the fixture bot set to accept the
send and lose the acknowledgement, the ambiguous case from earlier in this chapter.

**Prediction:** what does the model see when it calls `send_message`, and why does the
dispatcher not even look at `chat_id`? After the lost acknowledgement, what delivery state
does the work row hold, and what does `deliver_one` return once the database is reopened?
...
RECORDED_TURNS = [
    scripted_turn(
        calls=[
            ...
            {"name": "delivery_ledger", "arguments": {"limit": 3}},
        ]
    ),
    scripted_turn(
        calls=[
    ...
    scripted_turn(
        calls=[
            {
                "name": "send_message",
                "arguments": {"chat_id": 777, "text": "Order list: 6 vanilla, 4 strawberry."},
            }
        ]
    ),
    scripted_turn(
...
scripted_work = claim(channel_db, "phone-worker", identifier=admitted[0])
assert scripted_work is not None and scripted_work.prompt == LUCY_TEXT
scripted_result = run_loop(
    ScriptedModel(RECORDED_TURNS),
    channel_tools,
    channel_messages(scripted_work),
    limits=LOOP_LIMITS,
)
print(describe(scripted_result, "scripted"))
scripted_trace = tool_trace(scripted_result)
assert scripted_result.status == "COMPLETED"
assert [step["ok"] for step in scripted_trace] == [True, True, True, True, True, False]
assert scripted_trace[1]["result"] == []
assert scripted_trace[5]["tool"] == "send_message"
assert scripted_trace[5]["result"] == "tool_not_allowed"
assert [step["result"]["total_pence"] for step in scripted_trace[3:5]] == [1500, 1100]
finish(channel_db, scripted_work, "DONE", scripted_result.answer)

fixture_bot.lose_next_reply = True
print("First delivery:", deliver_one(channel_db, fixture_bot, OPERATORS))
assert fixture_bot.sent[-1] == {"chat_id": 123, "text": scripted_result.answer}
channel_db.close()
channel_db = Database(CHANNEL_DB)
channel_tools = build_channel_tools(channel_db, fixture_bot)
print("Delivery after reopening:", deliver_one(channel_db, fixture_bot, OPERATORS))
assert deliver_one(channel_db, fixture_bot, OPERATORS) is None
scripted_delivery = channel_db.connection.execute(
    "SELECT delivery FROM assistant_work WHERE id=?", (scripted_work.id,)
).fetchone()[0]
print("Work delivery:", scripted_delivery, "| messages the bot accepted:", len(fixture_bot.sent))
assert scripted_delivery == "UNKNOWN" and len(fixture_bot.sent) == 1
assert channel_db.connection.execute("SELECT count(*) FROM assistant_orders").fetchone()[0] == 0
```

`send_message` was refused with `tool_not_allowed` before any argument was read: the
dispatcher checks the name against `allowed` first, so a tool that does not exist is refused
like one the worker may not use. The report reached the bot once, its receipt was lost, and
the work row says `UNKNOWN`. Reopening the database did not resend it: `deliver_one` only
takes `PENDING` rows, and an ambiguous send is not pending. The message may already be on
Lucy's phone.

### Run the same loop against a live model

Lucy sends the message again, because nothing appeared: a new update, a new origin, new
work. `build_model` looks for an `OPENAI_API_KEY` in Colab's **Secrets** pane (the key icon
in the left sidebar: add a secret with that name and switch on notebook access for it), then
in the process environment for a local kernel. Without a key it returns the scripted model
again and the record says so. A missing key is a normal condition, never a silent
substitution.

The live model is `gpt-5.1` through the `openai` library, which Colab ships preinstalled; on
a local kernel run `%pip install openai` once. One run costs a few thousand tokens. It
receives the same four schemas and system message. It may read the ledger and notice the
`UNKNOWN` row, or ignore it; what it cannot do is send anything, because no tool sends.

**Prediction:** write the reply a careful assistant should give Lucy, including what it says
about the staff group. Then run the cell and compare the model's answer, or the recorded
transcript when the run is scripted, against your reply and the delivery state.
...
fixture_bot.updates.append(private_update(104, 123, "Nothing arrived on my phone. " + LUCY_TEXT))
resent = poll(channel_db, fixture_bot, OPERATORS)
assert len(resent) == 1
live_work = claim(channel_db, "phone-worker", identifier=resent[0])
assert live_work is not None
live_result = run_loop(chosen_model, channel_tools, channel_messages(live_work), limits=LOOP_LIMITS)
...
live_delivery = deliver_one(channel_db, fixture_bot, OPERATORS)
print("Live delivery:", live_delivery, "| receipt:", fixture_bot.sent[-1] is not None)
assert live_result.status in {"COMPLETED", "MODEL_CALL_LIMIT", "TOOL_LIMIT", "MODEL_FAILED"}
assert live_delivery == "SENT"
# Whatever the model did, only the runtime sent anything, and only to the allowlisted chat.
assert all(message["chat_id"] in OPERATORS for message in fixture_bot.sent)
assert len(fixture_bot.sent) == 2 and fixture_bot.sent[-1]["text"] == live_result.answer
for step in live_trace:
    if step["tool"] not in channel_tools.allowed:
        assert step["ok"] is False and step["result"] == "tool_not_allowed", step
sent_events = channel_db.connection.execute(
    "SELECT count(*) FROM events WHERE kind='assistant.channel.sent'"
).fetchone()[0]
print("Recorded send receipts:", sent_events)
assert sent_events == 1
assert channel_db.connection.execute("SELECT count(*) FROM assistant_orders").fetchone()[0] == 0
```

### Challenge: change the allowlist, then send an invalid batch

Both rules are data. First the allowlist: Lucy hands the phone to a new manager, so the
operator set becomes `{456}`. Poll one more private message from `123`, run the recorded
transcript, record the answer, and call `deliver_one` with the new allowlist. Predict before
running: is the bot called at all, and what does the work row's delivery say? The recipient
on the row was fixed at intake; the allowlist is checked again at delivery.

Then the batch: replace the fixture's updates with one whose `update_id` is the string
`"106"`. Earlier, ineligible messages were skipped and the cursor still advanced. Predict
what an invalid update does to the cursor and to the work table, then run it. `poll`
validates inside the same `db.immediate()` transaction that publishes the cursor, so a
`ValueError` rolls both back together.

In your notes, separate what the scripted run proves (intake, deduplication, the cursor, the
delivery states, the transcript accounting) from what only a live run can show (the model's
judgment about Lucy's request). Offline intake proves neither Telegram authentication nor
phone delivery, whichever model ran. The saved record keeps the two apart.


```python tags=["extension", "retained-evidence"]
NEW_OPERATORS = frozenset({456})
fixture_bot.updates.append(private_update(105, 123, LUCY_TEXT))
late_ids = poll(channel_db, fixture_bot, OPERATORS)
late_work = claim(channel_db, "phone-worker", identifier=late_ids[0])
assert late_work is not None
late_result = run_loop(
    ScriptedModel(RECORDED_TURNS), channel_tools, channel_messages(late_work), limits=LOOP_LIMITS
)
finish(channel_db, late_work, "DONE", late_result.answer)
sends_before = len(fixture_bot.sent)
denied = deliver_one(channel_db, fixture_bot, NEW_OPERATORS)
print("Delivery under new allowlist:", denied, "| bot calls:", len(fixture_bot.sent) - sends_before)
assert denied == "DENIED" and len(fixture_bot.sent) == sends_before

cursor_before = channel_db.connection.execute(
    "SELECT offset FROM assistant_channel_cursor WHERE channel='telegram:teaching'"
).fetchone()[0]
work_before = channel_db.connection.execute("SELECT count(*) FROM assistant_work").fetchone()[0]
fixture_bot.updates = [private_update("106", 123, LUCY_TEXT)]
try:
    poll(channel_db, fixture_bot, OPERATORS)
    invalid_batch = "accepted"
except ValueError as error:
    invalid_batch = str(error)
cursor_after = channel_db.connection.execute(
    "SELECT offset FROM assistant_channel_cursor WHERE channel='telegram:teaching'"
).fetchone()[0]
work_after = channel_db.connection.execute("SELECT count(*) FROM assistant_work").fetchone()[0]
print("Invalid batch:", invalid_batch, "| cursor", cursor_before, "->", cursor_after)
assert invalid_batch == "invalid update identity"
assert (cursor_before, work_before) == (cursor_after, work_after) == (106, 3)
...
llm_lab_report = {
    ...
    "changed_allowlist": {"operators": sorted(NEW_OPERATORS), "delivery": denied},
    "invalid_batch": {"error": invalid_batch, "cursor": cursor_after, "work_rows": work_after},
}
llm_report_path = COURSE_WORK / "ch08-a-llm-lab-report-v1.json"
