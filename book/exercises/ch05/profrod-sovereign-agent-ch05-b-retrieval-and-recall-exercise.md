...
jupyter:
  ...
  course:
    ...
    lesson_id: memory
    planned_minutes: 90
    resource_id: profrod-sovereign-agent-ch05-b-retrieval-and-recall-exercise
...
[![Open in Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/profrodai/sovereign-agent/blob/main/book/exercises/ch05/profrod-sovereign-agent-ch05-b-retrieval-and-recall-exercise.ipynb) Runs on Google Colab as it ships today (Python 3.13), or on any local Python 3.12+ kernel. It needs nothing beyond Python's standard library, and it makes no network call.

This is the second of Chapter 5's two practical units. Unit A made Lucy's preferences durable. This unit decides which stored notes reach the model.

A colleague has written the agent's memory search. It counts the words a note shares with the request and sends the top three notes. It "works on every question I tried". You will build BM25, the standard lexical ranking function, and measure both rankers against labeled questions. Then you will write the metric that says how early the right note appears.

By the end you should be able to:

1. Explain recall@k and reciprocal rank, and why recall is the measure that matters to an agent.
2. Derive BM25 from three ideas: rare terms matter more, repetition saturates, and long records are discounted.
3. Evaluate rankers against labeled questions, with a negative control, and separate direct questions from paraphrases.
4. Say what no lexical ranker can do, and what would.
...
def bm25_scores(query, documents, k1=1.5, b=0.75):
    ...
    wanted = set(terms(query))
    return [float(len(wanted & set(terms(d)))) for d in documents]
```
...
def mean_reciprocal_rank(rankings, relevant_sets):
    raise NotImplementedError("Score 1 / position of the first relevant index, then average")
```
