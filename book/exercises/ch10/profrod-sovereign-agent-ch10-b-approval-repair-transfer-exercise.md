...
Lucy has granted a standing automatic allowance of 1500 pence per order under a 2000 pence
...
TARGET = "lucy-local"
OPERATOR = "lucy"
SHOP_POLICY = SpendingPolicy(frozenset({OPERATOR}), total_pence=2000, automatic_order_pence=1500)
...
def exact_digest(sku, quantity, unit_cost_pence, target=TARGET):
...
def spending_row(db):
    ...
    if row is None:
        return {"limit_pence": None, "reserved_pence": 0, "spent_pence": 0}
    return {k: row[k] for k in ("limit_pence", "reserved_pence", "spent_pence")}
...
def build_approval_tools(db, work, policy):
    ...
    def propose_order(args):
        ...
        return {"sku": args.sku, "quantity": args.quantity, **dict(row), "currency": "GBP"}
...
assert scripted_spending == {"limit_pence": 2000, "reserved_pence": 1500, "spent_pence": 0}
...
assert live_spending["reserved_pence"] == sum(order["amount"] for order in approved)
assert live_spending["reserved_pence"] + live_spending["spent_pence"] <= SHOP_POLICY.total_pence
assert live_spending["spent_pence"] == 0  # nothing was ever sent to a supplier
for order in approved:
    assert order["amount"] <= SHOP_POLICY.automatic_order_pence
...
### Challenge: the exact limit, one pence over, and a stricter supplied policy

The ceiling is data. Rerun the recorded transcript on a fresh ledger with
`total_pence=2600`, which is exactly the two drafts together, and predict before running
whether the second approval is accepted: the rule is "at most", not "below". Then rerun with
`total_pence=2599` and predict which refusal returns.
...
def rerun_with(total_pence, name):
    db, work = open_shop(COURSE_WORK / f"ch10-llm-approval-{name}.sqlite")
    policy = SpendingPolicy(
        frozenset({OPERATOR}), total_pence=total_pence, automatic_order_pence=1500
    )
...
assert spending_row(exact_db)["reserved_pence"] == 2600  # reapproval reserved nothing new
stricter = SpendingPolicy(frozenset({OPERATOR}), total_pence=2000)
try:
...
assert spending_row(exact_db)["reserved_pence"] == 2600
