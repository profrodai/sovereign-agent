...
jupyter:
  ...
  course:
    ...
    planned_minutes: 90
    resource_id: profrod-sovereign-agent-ch15-b-evaluation-statistics-exercise
...
This is the second of Chapter 15's two practical units. Unit A built the baseline that the agent must beat. This unit asks what an evaluation result can claim.

A colleague reports two numbers. The agent passed 16 of 16 cases, "so it is 100% reliable, give or take nothing". A new prompt passed 12 of 14 cases against the old prompt's 10 of 14, "so it is 14 points better". Neither claim survives an error bar. You will construct the interval that evaluations should report, check it against its own promise with an exact calculation, and then build the paired test that compares two candidates on the same cases.

By the end you should be able to:

1. Explain why a pass rate is an estimate, and derive the Wilson interval from the score test.
2. Compute an interval's exact coverage, and show that the textbook Wald interval breaks for good agents on small suites.
3. Compare two candidates on the same cases with McNemar's exact test, and say why only the disagreements count.
4. Say how many cases a comparison needs before running it.
...
def wilson_interval(successes, trials, z=Z95):
    ...
    return wald_interval(successes, trials, z)
```
...
def mcnemar(first, second):
    raise NotImplementedError("Count the discordant cases, then sum the binomial tail")
```
