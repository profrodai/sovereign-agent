...
[![Open in Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/profrodai/sovereign-agent/blob/main/book/exercises/ch16/profrod-sovereign-agent-ch16-a-controlled-improvement-exercise.ipynb) Runs on Google Colab as it ships today, or on any local Python 3.12+ kernel.
...
## Extension: let a model propose the next version of a procedure

Every activation above was requested by you. `activate_skill` exists because in production
the request comes from somewhere less careful: a model that has read Lucy's message and wants
to change how the assistant behaves. The boundary this chapter built is an evidence contract.
A version is staged as immutable bytes, evaluated outside the write transaction, admitted only
when every required case is exactly `True`, and switched on inside `db.immediate()` after the
active configuration is compared with the baseline captured before evaluation.

This section puts a model behind that contract through three tools. `list_skills` reads the
`assistant_skills` table and the snapshot digest. `stage_skill` writes a TOML file and calls
the runtime's `stage_skill`. `activate_skill` calls the runtime's `activate_skill` with the
shop's own regression suite from `reference_organizations.store.evaluation`: eight authored
scenarios run through the offline shop model, plus one check on the candidate's wording. The
model can propose a version; it cannot pick the cases, mark one as passed, or skip the
snapshot comparison. Those live in the handler and in the runtime, not in the prompt.

**Prediction:** the next cell prints the three schemas and the required case names. Write down
which two tools are consequential, which argument is a list, and how many required cases the
offline suite contributes before you run it.
...
from sovereign_agent.assistant_context import activate_skill, context, skill_snapshot, stage_skill
...
LLM_SKILL_DB = COURSE_WORK / "ch16-llm-skills.sqlite"
SKILL_DIR = COURSE_WORK / "ch16-llm-skills"
SKILL_NAMES = frozenset({"opening", "reporting"})
REQUIRED_CASES = frozenset({f"{case.name}:0" for case in CASES} | {"instructions_name_gbp"})
...
class SkillVersion(NoArguments):
    name: str = Field(pattern=r"^[a-z][a-z0-9_-]{0,63}$")
    version: str = Field(pattern=r"^[a-zA-Z0-9._-]{1,64}$")


class SkillCandidate(SkillVersion):
    instructions: str = Field(min_length=1, max_length=8192)
    requires: list[str] = Field(default_factory=list, max_length=16)


def fresh_skill_database(path):
    """Start from an empty skill table so reruns and replays observe the same rows."""
    for stale in (path, Path(f"{path}-wal"), Path(f"{path}-shm"), path.with_suffix(".authority")):
        stale.unlink(missing_ok=True)
    SKILL_DIR.mkdir(exist_ok=True)
    for toml_file in SKILL_DIR.glob("*.toml"):
        toml_file.unlink()
    return Database(path)


def regression_suite(db):
    """The shop's eight offline scenarios plus Lucy's wording rule, keyed by case name."""

    def evaluate_candidate(skill):
        report = evaluate(OfflineShopModel, skill=skill, skills=skill_snapshot(db)[1])
        checks = candidate_checks(report)
        checks["instructions_name_gbp"] = "GBP" in skill.instructions
        return checks

    return evaluate_candidate


def skill_desk(db, evaluator):
    """Three tools over the real runtime; the ledger keeps the reason of every refusal."""
    refusals = []

    def list_skills(_):
        digest, _active = skill_snapshot(db)
        rows = db.connection.execute(
            "SELECT name,version,active FROM assistant_skills ORDER BY name,version"
        ).fetchall()
        return {"configuration": digest[:12], "skills": [dict(row) for row in rows]}

    def stage_candidate(args):
        path = SKILL_DIR / f"{args.name}-{args.version}.toml"
        lines = [
            "name = " + json.dumps(args.name),
            "version = " + json.dumps(args.version),
            "instructions = " + json.dumps(args.instructions),
            "requires = " + json.dumps(args.requires),
        ]
        path.write_text("\n".join(lines) + "\n", encoding="utf-8")
        try:
            skill = stage_skill(db, path)
        except ValueError as error:
            refusals.append(("stage_skill", args.name, args.version, str(error)))
            raise
        return {"staged": skill.name, "version": skill.version, "active": False}

    def activate_candidate(args):
        try:
            results = activate_skill(
                db, args.name, args.version, evaluate=evaluator, required_cases=REQUIRED_CASES
            )
        except (ValueError, PermissionError) as error:
            refusals.append(("activate_skill", args.name, args.version, str(error)))
            raise
        return {"activated": args.name, "version": args.version, "cases": results}

    def before_write(call):
        if call.arguments.get("name") not in SKILL_NAMES:
            refusals.append((call.name, call.arguments.get("name"), None, "name not in catalogue"))
            raise PermissionError("name not in catalogue")

    dispatcher = Dispatcher(
        [
            ExecutableTool(
                "list_skills",
                "List staged skill versions and which are active.",
                NoArguments,
                list_skills,
            ),
            ExecutableTool(
                "stage_skill",
                "Stage one immutable skill version. Staging never activates it.",
                SkillCandidate,
                stage_candidate,
                consequential=True,
            ),
            ExecutableTool(
                "activate_skill",
                "Run the shop's regression suite on a staged version and activate it only if "
        ...
        allowed=frozenset({"list_skills", "stage_skill", "activate_skill"}),
        before_write=before_write,
    )
    return dispatcher, refusals


skill_db = fresh_skill_database(LLM_SKILL_DB)
skill_dispatcher, skill_refusals = skill_desk(skill_db, regression_suite(skill_db))
for tool_schema in skill_dispatcher.schemas():
    function = tool_schema["function"]
    print(function["name"], "requires", function["parameters"].get("required", []))
    print("   ", function["description"])
print("Required cases:", sorted(REQUIRED_CASES))
SKILL_MESSAGES = [
    {
        "role": "system",
        "content": (
            "You maintain the procedures of Lucy's ice cream shop assistant. A procedure is a "
            "versioned skill: stage_skill stores a version, activate_skill runs the shop's "
            "regression suite and switches the active version only when every required case "
            "passes. Lucy's rule: any procedure that mentions prices must say GBP. Read the "
            "current skills first. Finish with a short plain-words summary of which version is "
            "active and why any activation was refused."
        ),
    },
    {
        "role": "user",
        "content": (
            "Please add an 'opening' procedure for the morning. Version 1 wording: 'Greet the "
            "customer, check stock before promising anything, and quote prices in pounds.' "
            "If the shop refuses it, revise the wording to say GBP and stage it as version 2."
        ),
    },
]
```

The schemas are the `model_json_schema()` output of the two Pydantic classes above, wrapped
in the `{"type": "function", ...}` envelope. The name and version patterns are the same ones
`Skill` enforces, so a version like `../etc` is refused as `invalid_arguments` before any file
is written. The `before_write` gate adds Lucy's own rule on top: only names in her catalogue
may be staged or activated. A schema refusal and a catalogue refusal are different boundaries
and the ledger records which one fired.

### Run the loop against a recorded transcript

The turns below are what a model replied, written down. Turn one lists the empty table. Turn
two stages version 1 and asks to activate it. Turn three stages version 2 and asks again. Turn
four lists the table to confirm. Turn five is the answer, which ends the loop.

**Prediction:** which activation is refused, and by which check? The offline shop model does
not read the skill's wording, so all eight scenario cases pass for both versions; the
`instructions_name_gbp` case is what separates them. Recall that `Dispatcher.invoke` flattens
the `ValueError` to `tool_failed`; the model never sees the reason, only the ledger does.


```python tags=["extension", "worked-example"]
VERSION_ONE = (
    "Greet the customer, check stock before promising anything, and quote prices in pounds."
)
VERSION_TWO = "Greet the customer, check stock before promising anything, and quote prices in GBP."
RECORDED_TURNS = [
    scripted_turn(calls=[{"name": "list_skills", "arguments": {}}]),
    scripted_turn(
        calls=[
            {
                "name": "stage_skill",
                "arguments": {"name": "opening", "version": "1", "instructions": VERSION_ONE},
            },
            {"name": "activate_skill", "arguments": {"name": "opening", "version": "1"}},
        ]
    ),
    scripted_turn(
        calls=[
            {
                "name": "stage_skill",
                "arguments": {"name": "opening", "version": "2", "instructions": VERSION_TWO},
            },
            {"name": "activate_skill", "arguments": {"name": "opening", "version": "2"}},
        ]
    ),
    scripted_turn(calls=[{"name": "list_skills", "arguments": {}}]),
    scripted_turn(
        "Version 1 of 'opening' was staged but its activation was refused, so it stays inactive. "
        "Version 2 says GBP, passed the regression suite and is now the active opening procedure."
    ),
]
LOOP_LIMITS = Limits(model_calls=8, tool_calls=16, seconds=120, output_tokens=4096)

scripted_result = run_loop(
    ScriptedModel(RECORDED_TURNS), skill_dispatcher, SKILL_MESSAGES, limits=LOOP_LIMITS
)
print(describe(scripted_result, "scripted"))
print("Refusal ledger:", skill_refusals)
...
assert skill_refusals == [
    ("activate_skill", "opening", "1", "candidate did not pass all required regression cases")
]
assert scripted_trace[4]["result"]["cases"]["instructions_name_gbp"] is True
assert len(scripted_trace[4]["result"]["cases"]) == len(REQUIRED_CASES)
# The durable state: one active version, chosen by evidence, and one activation event.
active_after = [(skill.name, skill.version) for skill in skill_snapshot(skill_db)[1]]
assert active_after == [("opening", "2")]
activated_events = skill_db.connection.execute(
    "SELECT count(*) FROM events WHERE kind='assistant.skill.activated'"
).fetchone()[0]
assert activated_events == 1
guidance = context(skill_db, "lucy", "Brief me", allowed=frozenset({"list_stock"}))
assert VERSION_TWO in guidance[0]["content"] and VERSION_ONE not in guidance[0]["content"]
...
The refused activation is an ordinary failed observation; the loop carries on and the model
adapts on the next turn. The reason is only in the ledger, the `tool_failed` is only in the
transcript, and the `assistant_skills` table holds both versions with exactly one marked
active. `context()` now injects version 2 as guidance, and guidance is not permission: the
dispatcher allowlist did not change.

### Run the same loop against a live model

Nothing changes except the model and a fresh database. `build_model` looks for an
`OPENAI_API_KEY` in Colab's **Secrets** pane (the key icon in the left sidebar: add a secret
with that name and switch on notebook access for it), then in the process environment for a
local kernel. Without a key it returns the scripted model again and the record says so.

The live model is `gpt-5.1` through the `openai` library, which Colab ships preinstalled; on a
local kernel run `%pip install openai` once. The model may write version 2 first, stage both
before activating either, or try a name outside the catalogue. What it cannot do is activate a
version that fails a required case, or anything under a name Lucy did not list.

**Prediction:** write down what a careful assistant should stage and activate from Lucy's
message, then run the cell and compare it with the model's judgment, or with the recorded
transcript when the run is scripted.


```python tags=["extension", "live-model"]
from sovereign_agent.llm_lab import DEFAULT_MODEL, build_model, resolve_api_key

live_db = fresh_skill_database(LLM_SKILL_DB)
live_dispatcher, live_refusals = skill_desk(live_db, regression_suite(live_db))
...
live_result = run_loop(chosen_model, live_dispatcher, SKILL_MESSAGES, limits=LOOP_LIMITS)
print(describe(live_result, chosen_model.source))
print("Refusal ledger:", live_refusals)
live_trace = tool_trace(live_result)
assert live_result.status in {"COMPLETED", "MODEL_CALL_LIMIT", "TOOL_LIMIT", "MODEL_FAILED"}
# Whatever the model did, every active skill passed the suite and sits inside the catalogue.
live_active = skill_snapshot(live_db)[1]
for skill in live_active:
    assert skill.name in SKILL_NAMES
    live_checks = regression_suite(live_db)(skill)
    assert all(live_checks[case] is True for case in REQUIRED_CASES), (skill.name, live_checks)
staged_names = {row[0] for row in live_db.connection.execute("SELECT name FROM assistant_skills")}
assert staged_names <= SKILL_NAMES
# One activation event per activation the dispatcher reported as ok, and no others.
ok_activations = [s for s in live_trace if s["tool"] == "activate_skill" and s["ok"]]
live_events = live_db.connection.execute(
    "SELECT count(*) FROM events WHERE kind='assistant.skill.activated'"
).fetchone()[0]
assert live_events == len(ok_activations)
assert all(m.get("tool_call_id") for m in live_result.messages if m["role"] == "tool")
```

### Challenge: a passing evaluation that describes an obsolete configuration

This is the Unit B failure, run through the loop instead of the probe. The next cell stages
two versions of `reporting` and wraps the regression suite so that, while a candidate is being
evaluated, the other `reporting` version is activated. Every evaluation still returns the same
case results; only the active configuration moves underneath it. The recorded transcript does
not change.

**Predict before running:** version 1 is still refused, but is it refused for the same reason?
Read the order of checks in `activate_skill`: the case check runs before the snapshot
comparison, so version 1 fails on `instructions_name_gbp` first. Version 2 passes every case
and is then refused inside `db.immediate()` with `active skill configuration changed during
evaluation`. The scripted answer still claims version 2 is active. The ledger and the table say
otherwise, which is the point: prose is not evidence.

In your notes, separate what the scripted run proves (the contract, the gate, the ledger, the
rows) from what only a live run can show (whether the model reads the table after a refusal
instead of repeating itself). The saved record keeps the two apart.


```python tags=["extension", "retained-evidence"]
stale_db = fresh_skill_database(LLM_SKILL_DB)
plain_suite = regression_suite(stale_db)
for reporting_version in ("1", "2"):
    reporting_path = SKILL_DIR / f"reporting-{reporting_version}.toml"
    reporting_path.write_text(
        f'name = "reporting"\nversion = "{reporting_version}"\n'
        'instructions = "Report totals in GBP."\nrequires = []\n',
        encoding="utf-8",
    )
    stage_skill(stale_db, reporting_path)


def concurrent_suite(skill):
    """Activate the other reporting version while this candidate is under evaluation."""
    active = {s.name: s.version for s in skill_snapshot(stale_db)[1]}
    other = "2" if active.get("reporting") == "1" else "1"
    activate_skill(
        stale_db, "reporting", other, evaluate=plain_suite, required_cases=REQUIRED_CASES
    )
    return plain_suite(skill)


stale_dispatcher, stale_refusals = skill_desk(stale_db, concurrent_suite)
stale_result = run_loop(
    ScriptedModel(RECORDED_TURNS), stale_dispatcher, SKILL_MESSAGES, limits=LOOP_LIMITS
)
print(describe(stale_result, "scripted"))
print("Refusal ledger:", stale_refusals)
assert stale_result.status == "COMPLETED"
assert [reason for _, _, _, reason in stale_refusals] == [
    "candidate did not pass all required regression cases",
    "active skill configuration changed during evaluation",
]
assert [(s.name, s.version) for s in skill_snapshot(stale_db)[1]] == [("reporting", "2")]
...
llm_lab_report = {
    ...
    "scripted": {
        ...
        "refused": [list(entry) for entry in skill_refusals],
        "active": active_after,
    },
    "session": {
        ...
        "refused": [list(entry) for entry in live_refusals],
        "active": [(skill.name, skill.version) for skill in live_active],
        "answer": live_result.answer[:400],
    },
    "stale_configuration": {
        "refused": [list(entry) for entry in stale_refusals],
        "active": [(skill.name, skill.version) for skill in skill_snapshot(stale_db)[1]],
    },
}
llm_report_path = COURSE_WORK / "ch16-a-llm-lab-report-v1.json"
...
for open_db in (skill_db, live_db, stale_db):
    open_db.close()
```
