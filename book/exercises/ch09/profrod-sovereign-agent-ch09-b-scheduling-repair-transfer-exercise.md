...
[![Open in Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/profrodai/sovereign-agent/blob/main/book/exercises/ch09/profrod-sovereign-agent-ch09-b-scheduling-repair-transfer-exercise.ipynb) Runs on Google Colab as it ships today, or on any local Python 3.12+ kernel.
...
## Extension: put a language model behind the scheduler

Every `schedule` and every `tick` above was called by you, with a clock you chose. In
production the caller who decides what Lucy's shop should wake up for is a model, and the
scheduler is the boundary that keeps that decision cheap: a schedule is a durable row, a pass
converts due rows into bounded work, and a missed weekend coalesces into one wakeup rather
than a backlog storm. This closing section puts a real model behind that boundary.

The mechanism is the loop from Chapter 3, shipped as `agent_loop.run_loop`. The model only
ever proposes a tool call. The dispatcher checks arguments against a Pydantic class; the
handler calls the same `schedule`, `unschedule` and `tick` from `assistant_work.py` that you
traced; a `before_write` gate can refuse a write before the handler runs. The model cannot
touch `assistant_jobs` directly, cannot invent a wall clock, and cannot make `tick` replay a
missed wakeup more than once. What it can do is choose identities, intervals and the clock it
hands to a pass, and those choices are what the transcript records.

The runtime adds one file for this section, `llm_lab.py`. `ScriptedModel` replays turns written
down in the notebook so the section runs identically for everyone; `OpenAIModel` sends the same
messages to a hosted model when a key is present. The record always says which one ran.

**Prediction:** the next cell builds four tools and prints their schemas. Write down which tools
are consequential, which argument the gate inspects, and why the shop's minimum interval is
thirty minutes when the daily intake allowance per session is fifty requests.
...
SCHEDULE_DB = COURSE_WORK / "ch09-llm-schedule.sqlite"
HOUR, DAY = 3600, 86400
SHOP_OPEN = 1_799_971_200  # Friday 00:00 UTC on the epoch clock; the clock is an input.
MONDAY_0900 = SHOP_OPEN + 3 * DAY + 9 * HOUR


def fresh_schedule_database(path):
...
class ScheduleArguments(NoArguments):
...
class JobArguments(NoArguments):
    identifier: str = Field(min_length=1, max_length=100)


class PassArguments(NoArguments):
    now: int = Field(ge=0)


def build_schedule_tools(db, *, minimum_interval=HOUR // 2):
    """Four tools over the chapter's scheduler, plus a ledger of every refusal reason."""
    refusals = []

    def list_schedules(_):
        rows = db.connection.execute(
            "SELECT id,prompt,interval_seconds,next_due,enabled,deferred "
            "FROM assistant_jobs ORDER BY id"
        )
    ...
    def schedule_job(args):
        try:
            schedule(
                ...
                "lucy",
                args.prompt,
                first_due=float(args.first_due),
                interval_seconds=args.interval_seconds,
            )
        except ValueError as error:
            refusals.append(("schedule_job", str(error)))
            raise
        return {"identifier": args.identifier, "next_due": args.first_due}

    def stop_job(args):
        try:
            unschedule(db, args.identifier)
        except ValueError as error:
            refusals.append(("stop_job", str(error)))
            raise
        return {"identifier": args.identifier, "enabled": 0}

    def run_pass(args):
        before = db.connection.execute("SELECT coalesce(max(seq),0) FROM events").fetchone()[0]
        created = tick(db, now=float(args.now))
        enqueued = db.connection.execute(
            "SELECT payload FROM events WHERE seq>? AND kind='assistant.job.enqueued' ORDER BY seq",
            (before,),
        ).fetchall()
        return {
            "now": args.now,
            "created": created,
            "coalesced": {
                json.loads(row["payload"])["job"]: json.loads(row["payload"])["coalesced"]
                for row in enqueued
            },
        }

    def before_write(call):
        interval = call.arguments.get("interval_seconds")
        if call.name == "schedule_job" and type(interval) is int and interval < minimum_interval:
            refusals.append(("schedule_job", "interval below the shop minimum"))
            raise PermissionError("interval below the shop minimum")

    tools = [
        ExecutableTool(
            "list_schedules",
            "Read every schedule row: identity, due time, enabled.",
            NoArguments,
            list_schedules,
        ),
        ExecutableTool(
            "schedule_job",
            "Create a repeating job. Identities are permanent; replace by stopping and creating.",
            ScheduleArguments,
            schedule_job,
            consequential=True,
        ),
        ExecutableTool(
            "stop_job",
            "Stop future ticks of a job; admitted work stays.",
            JobArguments,
            stop_job,
            consequential=True,
        ),
        ExecutableTool(
            "run_pass",
            "Run one scheduler pass at the given clock. Due jobs create one work item each.",
            PassArguments,
            run_pass,
            consequential=True,
        ),
    ...
    dispatcher = Dispatcher(
        tools, allowed=frozenset(tool.name for tool in tools), before_write=before_write
    )
    return dispatcher, refusals


schedule_db = fresh_schedule_database(SCHEDULE_DB)
schedule_dispatcher, schedule_refusals = build_schedule_tools(schedule_db)
for tool_schema in schedule_dispatcher.schemas():
    function = tool_schema["function"]
    print(function["name"], "requires", function["parameters"].get("required", []))
    print("   ", function["description"])
print("Consequential:", sorted(n for n, t in schedule_dispatcher.tools.items() if t.consequential))
SCHEDULE_MESSAGES = [
    {
        "role": "system",
        "content": (
            "You help Lucy run her ice cream shop's always-on assistant. Every time is an "
            f"integer of seconds on the shop's epoch clock; {SHOP_OPEN} is Friday 00:00 and "
            "the clock is an input you pass to a scheduler pass. Inspect the schedules before "
            "changing them. A schedule identity is permanent: to change a job, stop it and "
            "create a new identity. Finish with a short plain-words summary of what is "
            "scheduled and what the pass created."
        ),
    },
    {
        "role": "user",
        "content": (
            "Set up my morning brief at 07:00 every day and a stock check every hour. Then I "
            "changed my mind: make the brief 08:00, not 07:00. Finally show me what one "
            "scheduler pass creates if the laptop stays shut until Monday 09:00."
        ),
    },
]
```

The schemas are the `model_json_schema()` output you inspected earlier, wrapped in the
`{"type": "function", ...}` envelope; `strict=True` means `interval_seconds` as the string
`"3600"` is refused before any handler runs. Three tools are consequential, so the dispatcher
would refuse them outright if no gate were attached. The gate reads one argument only: an
interval under thirty minutes would consume the fifty-request daily allowance on its own
(86400 / 1800 is 48), and `_enqueue` would start rejecting Lucy's real requests.

### Run the loop against a recorded transcript

The turns below are what a model replied, written down. Turn one reads the empty schedule.
Turn two creates the 07:00 brief and the hourly check. Turn three tries to move the brief by
reusing its identity, which `schedule` refuses. Turn four stops the old job and creates
`morning-brief-0800`. Turn five runs one pass at Monday 09:00, after a weekend with no passes
at all. Turn six reads the schedules back, and turn seven answers.

**Prediction:** how many work items does the Monday pass create, and what does the record say
was coalesced for each job? Do the number-line arithmetic from the chapter first: the brief was
due Friday 08:00, the check Friday 01:00. Then write down where each `next_due` lands.


```python tags=["extension", "worked-example"]
def job(identifier, prompt, first_due, interval_seconds):
    return {
        "name": "schedule_job",
        "arguments": {
...
RECORDED_TURNS = [
    scripted_turn(calls=[{"name": "list_schedules", "arguments": {}}]),
    scripted_turn(
        calls=[
            job("morning-brief", "Morning brief for Lucy", SHOP_OPEN + 7 * HOUR, DAY),
            job("stock-check", "Check stock against reorder points", SHOP_OPEN + HOUR, HOUR),
        ]
    ),
    scripted_turn(
        calls=[job("morning-brief", "Morning brief for Lucy", SHOP_OPEN + 8 * HOUR, DAY)]
    ),
    scripted_turn(
        calls=[
            {"name": "stop_job", "arguments": {"identifier": "morning-brief"}},
            job("morning-brief-0800", "Morning brief for Lucy", SHOP_OPEN + 8 * HOUR, DAY),
        ]
    ),
    scripted_turn(calls=[{"name": "run_pass", "arguments": {"now": MONDAY_0900}}]),
    scripted_turn(calls=[{"name": "list_schedules", "arguments": {}}]),
    scripted_turn(
        "Scheduled morning-brief-0800 daily at 08:00 and stock-check hourly; the 07:00 brief is "
        "stopped. The Monday 09:00 pass created two work items: one brief covering the four "
        "missed mornings and one stock check covering the missed hours. Next due: Tuesday "
        "08:00 and Monday 10:00."
    ),
]
LOOP_LIMITS = Limits(model_calls=10, tool_calls=16, seconds=120, output_tokens=4096)


def job_rows(db):
    rows = db.connection.execute(
        "SELECT id,next_due,enabled,interval_seconds FROM assistant_jobs ORDER BY id"
    )
    return {row["id"]: dict(row) for row in rows}


scripted_result = run_loop(
    ScriptedModel(RECORDED_TURNS), schedule_dispatcher, SCHEDULE_MESSAGES, limits=LOOP_LIMITS
)
print(describe(scripted_result, "scripted"))
scripted_trace = tool_trace(scripted_result)
scripted_jobs = job_rows(schedule_db)
print("Refusals:", schedule_refusals)
print("Jobs:", json.dumps(scripted_jobs, indent=1))
assert scripted_result.status == "COMPLETED"
assert [step["ok"] for step in scripted_trace] == [True, True, True, False, True, True, True, True]
assert scripted_trace[3]["result"] == "tool_failed"
assert schedule_refusals == [
    ("schedule_job", "schedule identity already exists; use a new identity for a replacement")
]
monday_pass = scripted_trace[6]["result"]
assert len(monday_pass["created"]) == 2
assert monday_pass["coalesced"] == {"morning-brief-0800": 3, "stock-check": 80}
assert scripted_jobs["morning-brief"]["enabled"] == 0
assert scripted_jobs["morning-brief"]["next_due"] == SHOP_OPEN + 7 * HOUR
assert scripted_jobs["morning-brief-0800"]["next_due"] == SHOP_OPEN + 4 * DAY + 8 * HOUR
assert scripted_jobs["stock-check"]["next_due"] == MONDAY_0900 + HOUR
ready_work = schedule_db.connection.execute(
    "SELECT origin FROM assistant_work WHERE status='READY' ORDER BY origin"
).fetchall()
assert len(ready_work) == 2 and all(row["origin"].startswith("job:") for row in ready_work)
...
The refused reschedule appears in the transcript as an ordinary failed observation and the
loop carries on; the reason lives only in the notebook's own ledger, because `invoke` never
copies an error message into what the model sees. Three missed mornings collapsed into one
brief, and eighty missed hours into one check. The stopped 07:00 job kept its `next_due`
untouched: `unschedule` stops future ticks without rewriting history.
...
receives the same four schemas and system message. It may pick other identities or run the pass
at a different clock; what it cannot do is create an interval under thirty minutes or make a
pass advance a job by anything other than whole intervals, because those rules live in the gate
and in `tick`.

**Prediction:** write the schedule a careful assistant should leave behind, then run the cell
and compare the model's choices, or the recorded transcript when the run is scripted.
...
schedule_db.close()
live_db = fresh_schedule_database(SCHEDULE_DB)
live_dispatcher, live_refusals = build_schedule_tools(live_db)
...
live_result = run_loop(chosen_model, live_dispatcher, SCHEDULE_MESSAGES, limits=LOOP_LIMITS)
print(describe(live_result, chosen_model.source))
print("Refusals:", live_refusals)
live_trace = tool_trace(live_result)
live_jobs = job_rows(live_db)
print("Jobs:", json.dumps(live_jobs, indent=1))
assert live_result.status in {"COMPLETED", "MODEL_CALL_LIMIT", "TOOL_LIMIT", "MODEL_FAILED"}
# Whatever the model did, every job row came from an accepted schedule_job and the gate held.
accepted = {
    step["arguments"]["identifier"]: step["arguments"]
    for step in live_trace
    if step["tool"] == "schedule_job" and step["ok"]
}
assert set(live_jobs) == set(accepted)
for identifier, row in live_jobs.items():
    first_due = accepted[identifier]["first_due"]
    interval = accepted[identifier]["interval_seconds"]
    assert interval >= HOUR // 2 and row["interval_seconds"] == interval
    # A pass only ever advances by whole intervals, never by "one interval per missed wakeup".
    assert row["next_due"] >= first_due and (row["next_due"] - first_due) % interval == 0
for step in live_trace:
    if step["tool"] == "run_pass" and step["ok"]:
        assert len(step["result"]["created"]) == len(step["result"]["coalesced"])
        assert all(count >= 0 for count in step["result"]["coalesced"].values())
```

### Challenge: tighten the gate, then pause the runtime

Both rules are data. First rebuild the tools with `minimum_interval=2 * HOUR` and rerun the
recorded transcript. **Predict before running:** which call is refused now, in which order do
the two refusals appear in the ledger, and how many work items does the Monday pass create?
The transcript did not change; the gate did.

Then rerun on a fresh ledger whose control row is paused, which is the state `restore` leaves
behind in Chapter 18. `tick` honours that flag before it reads a single job. Predict what the
pass creates and whether `next_due` moves. Notice that the script's final sentence is now
wrong about what happened; the observations are right, and that is why the record keeps them
apart. With a live key, rerun the live cell under either change and explain what the model did
after its second job was refused.

In your notes, separate what the scripted run proves (the identity rule, the gate, coalescing
arithmetic, the pause flag, transcript accounting) from what only a live run can show (the
model's judgment about identities and clocks). The saved record keeps the two apart.


```python tags=["extension", "retained-evidence"]
tight_db = fresh_schedule_database(COURSE_WORK / "ch09-llm-schedule-tight.sqlite")
tight_dispatcher, tight_refusals = build_schedule_tools(tight_db, minimum_interval=2 * HOUR)
tight_result = run_loop(
    ScriptedModel(RECORDED_TURNS), tight_dispatcher, SCHEDULE_MESSAGES, limits=LOOP_LIMITS
)
tight_trace = tool_trace(tight_result)
print("Tight gate:", tight_result.status, "refusals:", tight_refusals)
assert [step["ok"] for step in tight_trace] == [True, True, False, False, True, True, True, True]
assert [reason for _, reason in tight_refusals] == [
    "interval below the shop minimum",
    "schedule identity already exists; use a new identity for a replacement",
]
assert tight_trace[6]["result"]["coalesced"] == {"morning-brief-0800": 3}

paused_db = fresh_schedule_database(COURSE_WORK / "ch09-llm-schedule-paused.sqlite")
with paused_db.immediate() as connection:
    connection.execute("UPDATE assistant_control SET paused=1 WHERE id=1")
paused_dispatcher, paused_refusals = build_schedule_tools(paused_db)
paused_result = run_loop(
    ScriptedModel(RECORDED_TURNS), paused_dispatcher, SCHEDULE_MESSAGES, limits=LOOP_LIMITS
)
paused_trace = tool_trace(paused_result)
print("Paused runtime:", paused_result.status, "pass created:", paused_trace[6]["result"])
assert paused_trace[6]["result"] == {"now": MONDAY_0900, "created": [], "coalesced": {}}
assert job_rows(paused_db)["morning-brief-0800"]["next_due"] == SHOP_OPEN + 8 * HOUR
...
llm_lab_report = {
    ...
    "scripted": {
        ...
        "refused": [reason for _, reason in schedule_refusals],
        "monday_pass": monday_pass["coalesced"],
        "next_due": {name: row["next_due"] for name, row in scripted_jobs.items()},
    },
    "session": {
        ...
        "refused": [reason for _, reason in live_refusals],
        "jobs": live_jobs,
        "answer": live_result.answer[:400],
    },
    "tight_gate": {
        "minimum_interval": 2 * HOUR,
        "refused": [reason for _, reason in tight_refusals],
        "monday_pass": tight_trace[6]["result"]["coalesced"],
    },
    "paused": {"created": paused_trace[6]["result"]["created"]},
}
llm_report_path = COURSE_WORK / "ch09-b-llm-lab-report-v1.json"
...
for open_db in (live_db, tight_db, paused_db):
    open_db.close()
```
