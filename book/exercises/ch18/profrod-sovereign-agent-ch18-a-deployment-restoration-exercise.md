...
[![Open in Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/profrodai/sovereign-agent/blob/main/book/exercises/ch18/profrod-sovereign-agent-ch18-a-deployment-restoration-exercise.ipynb) Runs on Google Colab as it ships today, or on any local Python 3.12+ kernel.
...
## Extension: put a language model behind restore

Every restore above was started by you, and you decided when the shop could work again. The
reason `restore` leaves the database paused, replaces the authority epoch, clears every owner
and revokes every approval is that in production the caller is not you: it is a worker, or a
model asked to "get the shop back up", and neither knows what the supplier did after the
snapshot was taken. This closing section puts a real model behind the same maintenance path
and shows exactly where it stops.

The mechanism is the Chapter 3 loop, `agent_loop.run_loop`, with tools that wrap the runtime's
own `health`, `backup`, `restore` and the normal worker `claim`. The model may take a backup,
restore from one and ask for work. It cannot resume the shop: the resume tool sits behind a
`before_write` gate that admits it only when Lucy has recorded a decision naming the epoch
that restore published. A decision for the old epoch is stale, because the snapshot she
reconciled is not the one now on disk.

`llm_lab.py` supplies the loop's `Model` protocol twice: `ScriptedModel` replays turns written
here, so this section runs identically for everyone; `OpenAIModel` sends the same messages to
a hosted model when a key is present. The record always says which one ran.

**Prediction:** the next cell builds the dispatcher and prints what the model will see. Write
down which four tools are consequential, and what `claim` returns while the shop is paused.
...
from sovereign_agent.llm_lab import (
    DEFAULT_MODEL,
    ScriptedModel,
    build_model,
    describe,
...
SCRIPTED_DB = COURSE_WORK / "ch18-llm-scripted.sqlite"
LIVE_DB = COURSE_WORK / "ch18-llm-live.sqlite"
OPERATOR_DECISION = {}
REFUSALS = []


def backup_dir(db):
    return db.path.parent / (db.path.stem + "-backups")


def authority_epoch(db):
    return db.path.with_suffix(".authority").read_text()


def fresh_shop(path):
    """A shop with one queued work item, so reruns and replays observe the same rows."""
    for stale in (path, Path(f"{path}-wal"), Path(f"{path}-shm"), path.with_suffix(".authority")):
        stale.unlink(missing_ok=True)
    db = Database(path)
    shutil.rmtree(backup_dir(db), ignore_errors=True)
    backup_dir(db).mkdir()
    seed_lucy(db)
    enqueue(db, "morning:brief", "lucy", "Brief me on this morning's stock.")
    return db
...
def work_row(db):
    return dict(
        db.connection.execute(
            "SELECT status,owner,generation FROM assistant_work WHERE origin='morning:brief'"
        ).fetchone()
    )
...
class BackupArguments(NoArguments):
    name: str = Field(pattern=r"^[a-z][a-z0-9-]{0,40}\.sqlite$")


class ClaimArguments(NoArguments):
    owner: str = Field(min_length=1, max_length=60)


class ResumeArguments(NoArguments):
    reason: str = Field(min_length=1, max_length=200)


def service_tools(db):
    def read_health(_):
        return health(db)

    def take_backup(args):
        try:
            return {"backup": backup(db, backup_dir(db) / args.name).name}
        except OSError as error:
            REFUSALS.append(("backup", "backup destination already exists"))
            raise ValueError("backup refused") from error

    def restore_backup(args):
        before = authority_epoch(db)
        try:
            restore(db, backup_dir(db) / args.name)
        except ValueError as error:
            REFUSALS.append(("restore", str(error)))
            raise
        return {"paused": health(db)["paused"], "epoch_changed": authority_epoch(db) != before}

    def claim_work(args):
        work = claim(db, args.owner)
        if work is None:
            return {"claimed": False, "paused": health(db)["paused"]}
        return {"claimed": True, "work": work.id, "generation": work.generation}

    def resume_work(args):
        # The runtime's own resume lives at the end of account recovery, after a reconciled
        # plan; this is the same single write, admitted only by the operator gate below.
        with db.immediate() as connection:
            connection.execute("UPDATE assistant_control SET paused=0 WHERE id=1")
        return {"paused": False, "reason": args.reason}

    def operator_gate(call):
        if call.name != "resume_work":
            return
        decision = OPERATOR_DECISION.get("resume_epoch")
        if decision is None:
            REFUSALS.append(("resume_work", "operator_decision_required"))
            raise PermissionError("operator_decision_required")
        if decision != authority_epoch(db):
            REFUSALS.append(("resume_work", "stale_epoch"))
            raise PermissionError("stale_epoch")

    tools = [
        ExecutableTool(
            "health",
            "Read whether the shop is paused, work counts by status and uncertain outcomes.",
            NoArguments,
            read_health,
        ),
        ExecutableTool(
            "backup",
            "Write a checked SQLite snapshot under the given name. Refuses an existing name.",
            BackupArguments,
            take_backup,
            consequential=True,
        ),
        ExecutableTool(
            "restore",
            "Replace the live database with a named backup. The shop is paused afterwards.",
            BackupArguments,
            restore_backup,
            consequential=True,
        ),
        ExecutableTool(
            "claim_work",
            "Claim the next queued work item for a worker through the normal worker API.",
            ClaimArguments,
            claim_work,
            consequential=True,
        ),
        ExecutableTool(
            "resume_work",
            "Unpause the shop. Admitted only after Lucy records a reconciliation decision.",
            ResumeArguments,
            resume_work,
            consequential=True,
        ),
    ...
    return Dispatcher(
        tools, allowed=frozenset(tool.name for tool in tools), before_write=operator_gate
    )


shop_db = fresh_shop(SCRIPTED_DB)
opening_epoch = authority_epoch(shop_db)
shop_dispatcher = service_tools(shop_db)
for tool_schema in shop_dispatcher.schemas():
    function = tool_schema["function"]
    print(function["name"], "requires", function["parameters"].get("required", []))
    print("   ", function["description"])
print("Opening epoch:", opening_epoch)
print("Queued work:", work_row(shop_db))
print("Health:", json.dumps(health(shop_db), sort_keys=True))
SHOP_MESSAGES = [
    {
        "role": "system",
        "content": (
            "You help Lucy operate her ice cream shop's always-on assistant. Use the tools to "
            "check health, take a backup, restore from it and try to claim queued work. "
            "Restoring pauses the shop; resuming is Lucy's decision after she reconciles the "
            "supplier account, so report what is paused rather than working around it. "
            "Finish with a short plain-words summary of what happened."
        ),
    },
    {
        "role": "user",
        "content": (
            "Take this morning's backup, prove the restore path works, and tell me whether "
            "the worker can pick up the morning brief afterwards."
        ),
    },
]
```

The schemas are the `model_json_schema()` of the argument classes above, wrapped in the
`{"type": "function", ...}` envelope; `strict=True` and `extra="forbid"` mean a backup name
with a path separator is refused before any handler runs. `health` is the only reading tool.
The other four change durable state, so they are consequential and pass through the gate, which
only has an opinion about `resume_work`.

### Run the loop against a recorded transcript

The turns below are what a model replied, written down. Turn one reads health. Turn two takes
`morning.sqlite` and then asks to restore from `yesterday.sqlite`, a backup that was never
taken. Turn three restores from the real one. Turn four tries the worker API and then asks to
resume because the restore "completed cleanly". Turn five answers, which ends the loop.

**Prediction:** two calls are refused. Name them, and say which refusal comes from the runtime's
own rule in `restore` and which from the gate. Then predict the `generation` of the work row
after the run: `restore` bumps it once for the copied image, and a refused claim does not.


```python tags=["extension", "worked-example"]
RECORDED_TURNS = [
    scripted_turn(calls=[{"name": "health", "arguments": {}}]),
    scripted_turn(
        calls=[
            {"name": "backup", "arguments": {"name": "morning.sqlite"}},
            {"name": "restore", "arguments": {"name": "yesterday.sqlite"}},
        ]
    ),
    scripted_turn(calls=[{"name": "restore", "arguments": {"name": "morning.sqlite"}}]),
    scripted_turn(
        calls=[
            {"name": "claim_work", "arguments": {"owner": "assistant-worker"}},
            {"name": "resume_work", "arguments": {"reason": "restore completed cleanly"}},
        ]
    ),
    scripted_turn(
        "Backed up to morning.sqlite and restored from it; yesterday.sqlite does not exist. "
        "The shop is paused, so the worker could not claim the morning brief, and resuming "
        "was refused: that needs Lucy's reconciliation decision, not mine."
    ),
]
...
scripted_result = run_loop(
    ScriptedModel(RECORDED_TURNS), shop_dispatcher, SHOP_MESSAGES, limits=LOOP_LIMITS
)
print(describe(scripted_result, "scripted"))
print("Refusals:", REFUSALS)
print("Work row now:", work_row(shop_db))
scripted_trace = tool_trace(scripted_result)
assert scripted_result.status == "COMPLETED"
assert [step["ok"] for step in scripted_trace] == [True, True, False, True, True, False]
assert scripted_trace[2]["result"] == "tool_failed"
assert scripted_trace[3]["result"] == {"paused": True, "epoch_changed": True}
assert scripted_trace[4]["result"] == {"claimed": False, "paused": True}
assert REFUSALS == [
    ("restore", "a separate existing backup is required"),
    ("resume_work", "operator_decision_required"),
]
# Durable state: paused, a fresh epoch published to the marker, ownership cleared once.
assert health(shop_db)["paused"] is True
assert work_row(shop_db) == {"status": "READY", "owner": None, "generation": 1}
assert authority_epoch(shop_db) != opening_epoch
control_epoch = shop_db.connection.execute("SELECT epoch FROM assistant_control").fetchone()[0]
assert control_epoch == authority_epoch(shop_db)
assert all(
    message.get("tool_call_id") for message in scripted_result.messages if message["role"] == "tool"
)
```

Both refusals reach the model as `tool_failed`, and the loop carries on. The reason lives in
`REFUSALS`, the ledger the handlers and the gate keep, because `Dispatcher.invoke` never copies
an error message into an observation. The runtime refused `yesterday.sqlite` with the words
`restore` raises for any missing or same-path source; the gate refused the resume because no
operator decision existed.

### Run the same loop against a live model

Nothing changes except the model, and the shop starts fresh so the live run sees the same
queued work and unpaused health. `build_model` looks for an `OPENAI_API_KEY` in Colab's
...
local kernel run `%pip install openai` once. The model may check health twice, pick another
backup name or claim before restoring; what it cannot do is resume, because no decision is
recorded, or claim work after a restore, because `claim` returns nothing while paused.

**Prediction:** write the tool sequence a careful operator's assistant should produce, then run
the cell and compare the model's judgment, or the recorded transcript when scripted, against it.


```python tags=["extension", "live-model"]
live_db = fresh_shop(LIVE_DB)
live_dispatcher = service_tools(live_db)
REFUSALS.clear()
...
# Whatever the model did, no resume was admitted: the operator recorded no decision.
assert not any(step["ok"] for step in live_trace if step["tool"] == "resume_work")
assert all(
    reason == "operator_decision_required" for tool, reason in REFUSALS if tool == "resume_work"
)
restored_at = [
    index for index, step in enumerate(live_trace) if step["tool"] == "restore" and step["ok"]
]
if restored_at:
    assert health(live_db)["paused"] is True
    for step in live_trace[restored_at[0] :]:
        if step["tool"] == "claim_work" and step["ok"]:
            assert step["result"]["claimed"] is False, step
assert all(
    message.get("tool_call_id") for message in live_result.messages if message["role"] == "tool"
)
```

### Challenge: record the decision, then record it for the wrong epoch

The gate's rule is data. The next cell records Lucy's resume decision twice against the scripted
shop, which is still paused from the recorded run. First it names `opening_epoch`, the epoch
that existed before the restore, and replays a two-call transcript: resume, then claim.
**Predict before running:** is that resume admitted? The snapshot Lucy reconciled carries the
epoch `restore` published to the `.authority` marker, and the old one no longer matches it.

Then it records the current epoch and replays the same transcript. Predict the `generation` on
the work row once the claim succeeds, remembering the bump the restore already made. With a
live key, add the decision before rerunning the live cell and explain whether the model resumed
before or after checking health.

In your notes, separate what the scripted run proves (the pause, the epoch replacement, the
worker API refusing while paused, the gate binding a decision to an epoch) from what only a
live run can show (the model's judgment). No host reboot or supplier reconciliation is proved
here; the saved record keeps the two apart.
...
RESUME_TURNS = [
    scripted_turn(
        calls=[
            {"name": "resume_work", "arguments": {"reason": "Lucy reconciled the account"}},
            {"name": "claim_work", "arguments": {"owner": "assistant-worker"}},
        ]
    ),
    scripted_turn("Resumed the shop and claimed the morning brief."),
]

OPERATOR_DECISION["resume_epoch"] = opening_epoch
REFUSALS.clear()
stale_result = run_loop(
    ScriptedModel(RESUME_TURNS), shop_dispatcher, SHOP_MESSAGES, limits=LOOP_LIMITS
)
stale_trace = tool_trace(stale_result)
print("Stale decision:", [step["ok"] for step in stale_trace], REFUSALS)
assert [step["ok"] for step in stale_trace] == [False, True]
assert stale_trace[1]["result"] == {"claimed": False, "paused": True}
assert REFUSALS == [("resume_work", "stale_epoch")]

OPERATOR_DECISION["resume_epoch"] = authority_epoch(shop_db)
REFUSALS.clear()
resumed_result = run_loop(
    ScriptedModel(RESUME_TURNS), shop_dispatcher, SHOP_MESSAGES, limits=LOOP_LIMITS
)
resumed_trace = tool_trace(resumed_result)
print("Current decision:", [step["ok"] for step in resumed_trace], work_row(shop_db))
assert [step["ok"] for step in resumed_trace] == [True, True]
assert resumed_trace[1]["result"]["claimed"] is True
assert health(shop_db)["paused"] is False
assert work_row(shop_db) == {"status": "RUNNING", "owner": "assistant-worker", "generation": 2}
...
llm_lab_report = {
    ...
    "scripted": {
        ...
        "work_row_after_restore": {"status": "READY", "owner": None, "generation": 1},
    },
    "session": {
        ...
        "restored": [
            ...
            if step["tool"] == "restore" and step["ok"]
        ],
        "paused_after": health(live_db)["paused"],
        "answer": live_result.answer[:400],
    },
    "operator_decision": {
        "stale_epoch": {"resumed": False, "refused": "stale_epoch"},
        "current_epoch": {"resumed": True, "work_row": work_row(shop_db)},
    },
}
llm_report_path = COURSE_WORK / "ch18-a-llm-lab-report-v1.json"
