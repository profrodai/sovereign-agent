...
jupyter:
  ...
  course:
    ...
    planned_minutes: 90
    resource_id: profrod-sovereign-agent-ch07-a-durable-work-inbox-exercise
...
[![Open in Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/profrodai/sovereign-agent/blob/main/book/exercises/ch07/profrod-sovereign-agent-ch07-a-durable-work-inbox-exercise.ipynb) Runs on Google Colab as it ships today (Python 3.13), or on any local Python 3.12+ kernel. It needs nothing beyond Python's standard library.

This is the first of Chapter 7's two practical units, each a complete ninety-minute session.

- **Unit A** builds the inbox: requests admitted once per occurrence, bounded before any work runs, and moved through states that the database itself enforces.
- **Unit B** finishes work together with its report, and sends reports across a network that can lose a reply.

You bring Chapter 4's tables, constraints and transactions. Triggers, state machines and Little's law are introduced below, before the main task.

By the end you should be able to:

1. Predict what a restarted program knows about the requests it had accepted, and check the prediction.
2. Tell the text of a request from the identity of its occurrence, and say when a repeat is a duplicate and when it is a conflict.
3. Implement `admit`, which decides and records one request in one transaction and refuses work before storing anything when the inbox is full.
4. Make the database refuse a state transition that the state machine does not name, and read a capacity through Little's law, $L = \lambda W$.
...
def admit(connection, request, capacity):
    ...
    (waiting,) = connection.execute("SELECT COUNT(*) FROM work WHERE state = 'pending'").fetchone()
    if waiting >= capacity:
        return "refused"
    connection.execute(
...
def transfer_admit(open_work, request, budget):
    raise NotImplementedError("Decide identity first, then whether the minutes fit")
```
