...
[![Open in Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/profrodai/sovereign-agent/blob/main/book/exercises/ch12/profrod-sovereign-agent-ch12-b-recovery-repair-transfer-exercise.ipynb) Runs on Google Colab as it ships today, or on any local Python 3.12+ kernel.
...
## Extension: put a language model behind a claim that can go stale

`assert_current` above answered one question before every durable write: does this process
still own this work? It read the control row and the marker, compared epochs, matched the whole
claim identity against an unexpired lease, and refused on any mismatch. In production the
process holding the claim is running a model, and the model has no idea that a restore happened
in another terminal while it was composing its next reply.

This closing section puts a model behind that fence. Reads go straight to the ledger; the one
durable write, `record_finding`, appends a line to the work item's transcript through `observe`,
which calls `assert_current` inside its transaction. The `before_write` gate is the same
`assert_current`, called once more so the notebook keeps the runtime's own refusal sentence,
which the dispatcher would otherwise flatten to `tool_failed`. A model can read after it has
lost authority. It cannot write, and nothing it says can restore its claim: only a fresh `claim`
under the current epoch can.

**Prediction:** the next cell builds the dispatcher and prints the schemas. Write down which tool
is consequential, which fields `assert_current` matches against the `assistant_work` row, and
what a stale worker will still be able to observe through the read tools.
...
from sovereign_agent.assistant_work import assert_current, claim, enqueue, observe
...
class FindingArguments(NoArguments):
...
class RecoveryLab:
    ...
    def work_row(self):
        return dict(
            self.db.connection.execute(
                "SELECT generation,owner,status FROM assistant_work WHERE id=?", (self.work.id,)
            ).fetchone()
        )

    def findings(self):
        return [
            (row["generation"], json.loads(row["message"])["content"])
            for row in self.db.connection.execute(
                "SELECT generation,message FROM assistant_transcript WHERE work_id=? ORDER BY seq",
                (self.work.id,),
            )
        ]


def build_tools(lab, work):
    """Tools for one claim: two reads, one fenced durable write."""
    db = lab.db

    def authority_gate(call):
        try:
            assert_current(db.connection, work)
        except PermissionError as error:
            lab.ledger.append((work.generation, call.name, str(error)))
            raise
        lab.ledger.append((work.generation, call.name, "allowed"))

    def record_finding(args):
        observe(db, work, {"role": "finding", "content": args.text})
        return {"recorded": args.text, "generation": work.generation}

    def work_status(_):
        control = lab.control()
        return {
            "paused": bool(control["paused"]),
            "claim_epoch_current": control["epoch"] == work.epoch,
            "claim_generation": work.generation,
            "work": lab.work_row(),
            "transcript_rows": len(lab.findings()),
        }
    ...
    tools = [
        ...
        ExecutableTool(
            "work_status",
            "Read whether this worker's claim is still current and what the work row holds.",
            NoArguments,
            work_status,
        ),
        ExecutableTool(
            "record_finding",
            "Append one short finding to the work item's durable transcript.",
            FindingArguments,
            record_finding,
            consequential=True,
        ),
    ...
    return Dispatcher(
        tools, allowed=frozenset(tool.name for tool in tools), before_write=authority_gate
    )


def open_work(tag):
    """A fresh shop ledger with one work item claimed by this kernel."""
    db = Database(fresh_file(COURSE_WORK / f"ch12-{tag}-shop.sqlite"))
    seed_lucy(db)
    enqueue(db, f"ch12:{tag}", "lucy", "Check the shop this morning")
    work = claim(db, "notebook-worker", ttl=3600)
    lab = RecoveryLab(db, work, fresh_file(COURSE_WORK / f"ch12-{tag}-backup.sqlite"))
    lab.dispatcher = build_tools(lab, work)
    return lab


scripted_lab = open_work("scripted")
...
print("Claim:", scripted_lab.work.owner, "generation", scripted_lab.work.generation)
print("Control row:", scripted_lab.control()["paused"], "paused |", scripted_lab.work_row())
WORK_MESSAGES = [
    {
        "role": "system",
        "content": (
            "You help Lucy run her ice cream shop. Read stock, then record each product that "
            "needs attention as one short finding. If a finding is refused, do not claim it was "
            "recorded: read work_status and tell Lucy what it shows. Finish with a short summary."
        ),
    },
    {"role": "user", "content": "Check the shop and record what needs attention this morning."},
]
```

Three tools, one consequential. `record_finding` accepts a single bounded `text` field;
`extra="forbid"` means any extra field is refused before the gate or the handler runs. The gate
and the handler both call `assert_current` with the same claim; a current claim passes twice, a
stale one fails at the gate and the ledger keeps the runtime's own sentence.

### Run the recorded transcript twice: before and after a restore

Turn one reads stock. Turn two records a finding. Turn three records one with an extra
`urgent` field. Turn four records the corrected finding. Turn five reads `work_status`. Turn
six answers. The first run uses the fresh claim. Then Lucy takes a backup with the real
`backup`, and restores it with the real `restore`: a new epoch, a new marker, the work row reset
to `READY` with its generation raised, the control row paused. This kernel still holds the old
`Claim`, and the same six turns run again through it.

**Prediction:** for the first run, which call is refused and does the gate ever see it? For the
second run, which calls succeed, which are refused, what sentence does the gate record, and how
many transcript rows exist afterwards? The recorded final answer is the same both times. Decide
before running whether it is true both times.
...
RECORDED_TURNS = [
    ...
    scripted_turn(
        calls=[{"name": "record_finding", "arguments": {"text": "Vanilla is at 2 tubs; 6 needed."}}]
    ),
    scripted_turn(
        calls=[
            {
                "name": "record_finding",
                "arguments": {"text": "Strawberry is at 1 tub; 4 needed.", "urgent": True},
            }
        ]
    ),
    scripted_turn(
        calls=[
            {"name": "record_finding", "arguments": {"text": "Strawberry is at 1 tub; 4 needed."}}
        ]
    ),
    scripted_turn(calls=[{"name": "work_status", "arguments": {}}]),
    scripted_turn(
        "Vanilla and strawberry are below their reorder points; chocolate is fine. I submitted "
        "two findings. The work_status observation above shows whether the ledger accepted them."
    ),
]
LOOP_LIMITS = Limits(model_calls=8, tool_calls=16, seconds=120, output_tokens=4096)

scripted_result = run_loop(
    ScriptedModel(RECORDED_TURNS), scripted_lab.dispatcher, WORK_MESSAGES, limits=LOOP_LIMITS
)
...
assert [step["ok"] for step in scripted_trace] == [True, True, False, True, True]
assert scripted_trace[2]["result"] == "invalid_arguments"
# The schema refusal never reached the gate: two gate decisions, both allowed.
assert [decision for _, _, decision in scripted_lab.ledger] == ["allowed", "allowed"]
assert scripted_lab.findings() == [
    (1, "Vanilla is at 2 tubs; 6 needed."),
    (1, "Strawberry is at 1 tub; 4 needed."),
]
assert scripted_trace[4]["result"]["claim_epoch_current"] is True

# Lucy backs the ledger up, then restores it. This kernel still holds the old claim.
old_epoch = scripted_lab.control()["epoch"]
backup(scripted_lab.db, scripted_lab.backup_path)
restore(scripted_lab.db, scripted_lab.backup_path)
print(
    "Epoch changed:",
    scripted_lab.control()["epoch"] != old_epoch,
    "| work row:",
    scripted_lab.work_row(),
)
stale_result = run_loop(
    ScriptedModel(RECORDED_TURNS), scripted_lab.dispatcher, WORK_MESSAGES, limits=LOOP_LIMITS
)
print(describe(stale_result, "scripted, stale claim"))
print("Gate ledger:", scripted_lab.ledger[2:])
stale_trace = tool_trace(stale_result)
assert stale_result.status == "COMPLETED"
assert [step["ok"] for step in stale_trace] == [True, False, False, False, True]
assert stale_trace[1]["result"] == "tool_failed" and stale_trace[3]["result"] == "tool_failed"
assert [decision for _, _, decision in scripted_lab.ledger[2:]] == [
    "runtime paused or replaced by restore",
    "runtime paused or replaced by restore",
]
# The reads still worked; the writes did not; the transcript is exactly what the snapshot held.
assert stale_trace[4]["result"] == {
    "paused": True,
    "claim_epoch_current": False,
    "claim_generation": 1,
    "work": {"generation": 2, "owner": None, "status": "READY"},
    "transcript_rows": 2,
}
assert len(scripted_lab.findings()) == 2
assert stale_result.answer == scripted_result.answer  # same prose, different truth
```

Same transcript, same prose, different truth. The first run wrote two rows under generation 1;
the schema refusal never reached the gate. After the restore the reads still answered, both
writes were refused with the runtime's own sentence, and `work_status` showed why: paused, epoch
no longer current, work row at generation 2 with no owner. The recorded final sentence defers
to that observation, the only honest move for a model that cannot tell from inside whether its
claim is still good.
...
never a silent substitution. The live model is `gpt-5.1` through the `openai` library, which
Colab ships preinstalled; on a local kernel run `%pip install openai` once.

The live run uses a fresh ledger and claim, so every finding is admitted. What it proves is the
accounting: every transcript row carries this claim's generation, and there are exactly as many
rows as successful `record_finding` calls. The wording of the findings is the model's judgment.

**Prediction:** write the findings a careful assistant should record from the stock above.
Then run the cell and compare, or read the recorded transcript when the run is scripted.


```python tags=["extension", "live-model"]
from sovereign_agent.llm_lab import DEFAULT_MODEL, build_model, resolve_api_key

live_lab = open_work("live")
...
live_result = run_loop(chosen_model, live_lab.dispatcher, WORK_MESSAGES, limits=LOOP_LIMITS)
print(describe(live_result, chosen_model.source))
print("Transcript rows:", live_lab.findings())
live_trace = tool_trace(live_result)
assert live_result.status in {"COMPLETED", "MODEL_CALL_LIMIT", "TOOL_LIMIT", "MODEL_FAILED"}
# A current claim is never refused by the fence, whatever the model asks.
assert all(decision == "allowed" for _, _, decision in live_lab.ledger)
assert live_lab.control()["paused"] == 0
# One transcript row per admitted write, each stamped with this claim's generation.
recorded = [step for step in live_trace if step["tool"] == "record_finding" and step["ok"]]
assert len(live_lab.findings()) == len(recorded)
assert all(generation == live_lab.work.generation for generation, _ in live_lab.findings())
assert [text for _, text in live_lab.findings()] == [step["arguments"]["text"] for step in recorded]
assert all(
    message.get("tool_call_id") for message in live_result.messages if message["role"] == "tool"
)
```

### Challenge: resume, reclaim, and keep the old claim out

The restored ledger is paused and owned by nobody. Recovery is not replaying the model: Lucy
resumes the control row (the full recovery entry point, which also reconciles the supplier
account, is a later chapter; here the operator step is one row), and a fresh `claim` takes the
work at generation 3. The next cell runs the recorded transcript through that fresh claim and
then tries the old claim, an expired lease, a missing marker and a changed role against
`assert_current` directly, counting transcript rows after every attempt.

**Predict before running:** how many rows carry generation 1 and how many carry generation 3?
Which sentence refuses the old claim now that the ledger is no longer paused, and is it the
same sentence as before? Which refusals share a sentence, and why does the fence not say more?

In your notes, separate what the scripted runs prove (the fence, the generation stamps, the
four refusals) from what only a live run can show (whether the model reads `work_status` when a
write fails, or asserts that it recorded something it did not). The saved record keeps the two
apart, and the surviving limit stays: this fence cannot recall a request already admitted
before the replacement.


```python tags=["extension", "retained-evidence"]
with scripted_lab.db.immediate() as connection:
    connection.execute("UPDATE assistant_control SET paused=0 WHERE id=1")
replacement = claim(scripted_lab.db, "replacement-worker", ttl=3600)
assert replacement is not None and replacement.generation == 3
fresh_dispatcher = build_tools(scripted_lab, replacement)
fresh_result = run_loop(
    ScriptedModel(RECORDED_TURNS), fresh_dispatcher, WORK_MESSAGES, limits=LOOP_LIMITS
)
print("Fresh claimant:", fresh_result.status, "| rows:", scripted_lab.findings())
assert [step["ok"] for step in tool_trace(fresh_result)] == [True, True, False, True, True]
assert [generation for generation, _ in scripted_lab.findings()] == [1, 1, 3, 3]

refusals = {}
marker = scripted_lab.db.path.with_suffix(".authority")
attempts = {
    "stale_epoch": lambda: assert_current(scripted_lab.db.connection, scripted_lab.work),
    "expired_lease": lambda: assert_current(
        scripted_lab.db.connection, replacement, now=time.time() + 3601
    ),
    "changed_role": lambda: assert_current(
        scripted_lab.db.connection, replace(replacement, role="research")
    ),
    "missing_marker": lambda: assert_current(scripted_lab.db.connection, replacement),
}
epoch_text = marker.read_text()
for name, attempt in attempts.items():
    ...
    try:
        ...
        if name == "missing_marker":
            marker.write_text(epoch_text)
    assert len(scripted_lab.findings()) == 4, name
print("Refusals:", json.dumps(refusals, indent=2))
assert refusals == {
    "stale_epoch": "runtime paused or replaced by restore",
    "expired_lease": "worker claim expired or superseded",
    "changed_role": "worker claim expired or superseded",
    "missing_marker": "authority marker is unavailable",
}
assert_current(scripted_lab.db.connection, replacement)  # the fresh claimant still passes

llm_lab_report = {
    "unit": "ch12-b",
    "scripted": {
        "status": scripted_result.status,
        "refused": [step["result"] for step in scripted_trace if not step["ok"]],
        "gate": [list(entry) for entry in scripted_lab.ledger[:2]],
        "rows_after": 2,
    },
    "stale_claim": {
        "status": stale_result.status,
        "gate": [list(entry) for entry in scripted_lab.ledger[2:4]],
        "work_status": stale_trace[4]["result"],
    },
    "session": {
        ...
        "findings": [text for _, text in live_lab.findings()],
        "generation": live_lab.work.generation,
        "answer": live_result.answer[:400],
    },
    "recovery": {
        "replacement_generation": replacement.generation,
        "rows_by_generation": [generation for generation, _ in scripted_lab.findings()],
        "refusals": refusals,
    },
}
llm_report_path = COURSE_WORK / "ch12-b-llm-lab-report-v1.json"
...
for open_lab in (scripted_lab, live_lab):
    open_lab.db.close()
```
