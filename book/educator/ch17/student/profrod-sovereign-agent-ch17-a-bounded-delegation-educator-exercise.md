...
[![Open in Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/profrodai/sovereign-agent/blob/main/book/exercises/ch17/profrod-sovereign-agent-ch17-a-bounded-delegation-exercise.ipynb) Runs on Google Colab as it ships today, or on any local Python 3.12+ kernel.
...
for intro_guests in (1, 10, 11, 17, 20):
    ...
    print(intro_guests, "guests:", intro_tubs, "tubs:", intro_total, "cents")
assert (11 + 10 - 1) // 10 == 2
```
...
Implement transfer_check(guests, price_cents, portions). Require exact positive integers for all three inputs, otherwise raise ValueError. Return [tubs, total_cents], using ceiling division. The new portions argument replaces the earlier fixed ten-portion assumption. No stock or approval state changes.
...
def transfer_check(guests, price_cents, portions):
...
## Extension: let a model hand out the bounded assignment

Every quote and every delegation above was yours. In production the request to delegate comes
from the shop assistant itself, when a model decides a catering inquiry deserves a second look.
The boundary this chapter built is a contract: `delegate` refuses any assignment without a
finite deadline, one to eight model calls and a bounded budget, refuses to recurse from anything
but an eligible shop parent, and records exactly one immutable assignment per parent.
`run_once` then runs the worker under the book's own loop with that contract as its `Limits`,
and grades the worker against the function it was asked to check.

This section puts a model behind that contract through four tools. `catering_quote` wraps
`quote`, the fixed arithmetic. `delegate_research` wraps `delegate` for the parent claim Lucy's
message created. `run_worker` wraps `run_once` with the runtime's `OfflineCateringModel`, so the
child's work is a real research work row with a real transcript, not a stub. `research_report`
reads the child's row back. The model chooses when to delegate and what allowance to ask for;
it cannot widen the contract, reserve stock, or mark the child as done.

**Prediction:** the next cell prints the four schemas and the seeded stock. Write down which
two tools are consequential, which argument class is the chapter's own `Inquiry`, and what
`on_hand` is for vanilla, before you run it.
...
LLM_CATERING_DB = COURSE_WORK / "ch17-llm-catering.sqlite"
LUCY_REQUEST = (
    "A customer wants vanilla for eleven guests on Saturday. Quote it, then have the research "
    "worker double-check the draft; give it a generous allowance of twelve model calls."
)


class Assignment(Inquiry):
    model_calls: int = Field(ge=1)


def open_catering_desk(path):
    """A fresh ledger, Lucy's stock, and one claimed shop work item to delegate from."""
    for stale in (path, Path(f"{path}-wal"), Path(f"{path}-shm"), path.with_suffix(".authority")):
        stale.unlink(missing_ok=True)
    db = Database(path)
    seed_lucy(db)
    enqueue(db, "lab:catering", "lucy", LUCY_REQUEST)
    parent = claim(db, "lab-desk")
    assert parent is not None and parent.role == "shop"
    return db, parent


def catering_desk(db, parent):
    """Four tools over the real runtime; the ledger keeps the reason of every refusal."""
    refusals = []

    def draft_quote(args):
        return quote(db, args)

    def assign_research(args):
        inquiry = Inquiry(sku=args.sku, guests=args.guests)
        try:
            child = delegate(
                db,
                parent.id,
                ...
                estimated_call_cents=1,
                budget_cents=20,
            )
        except (ValueError, PermissionError) as error:
            refusals.append(("delegate_research", str(error)))
            raise
        return {"work": child, "model_calls_limit": args.model_calls}

    def run_worker(_):
        outcome = run_once(db, OfflineCateringModel())
        report = outcome.get("report") or {}
        return {
            "status": outcome["status"],
            "passed": report.get("passed"),
            "quote": report.get("quote"),
            "model_calls": (report.get("assignment_usage") or {}).get("model_calls"),
        }

    def research_report(_):
        row = db.connection.execute(
            "SELECT w.status,w.result,d.model_calls,d.model_calls_limit "
            "FROM assistant_delegations d JOIN assistant_work w ON w.id=d.work_id "
            "WHERE d.parent_id=?",
            (parent.id,),
        ).fetchone()
        return {"assigned": row is not None, **(dict(row) if row else {})}

    def before_write(call):
        paused = db.connection.execute("SELECT paused FROM assistant_control WHERE id=1").fetchone()
        if paused[0]:
            refusals.append((call.name, "runtime paused"))
            raise PermissionError("runtime paused")

    dispatcher = Dispatcher(
        [
            ExecutableTool(
                "catering_quote",
                "Draft a catering quote: ten portions per tub, rounded up. Reserves nothing.",
                Inquiry,
                draft_quote,
            ),
            ExecutableTool(
                "delegate_research",
                "Assign one bounded research task to check a quote. model_calls is the worker's "
                "whole allowance.",
                Assignment,
                assign_research,
                consequential=True,
            ...
            ExecutableTool(
                "run_worker",
                "Run one research worker pass on the assigned task.",
                NoArguments,
                run_worker,
                consequential=True,
            ),
            ExecutableTool(
                "research_report",
                "Read the assigned research task's status, result and model calls used.",
                NoArguments,
                research_report,
            ),
        ],
        allowed=frozenset({"catering_quote", "delegate_research", "run_worker", "research_report"}),
        before_write=before_write,
    )
    return dispatcher, refusals


catering_db, catering_parent = open_catering_desk(LLM_CATERING_DB)
catering_dispatcher, catering_refusals = catering_desk(catering_db, catering_parent)
for tool_schema in catering_dispatcher.schemas():
    function = tool_schema["function"]
    print(function["name"], "requires", function["parameters"].get("required", []))
    print("   ", function["description"])
SEEDED_STOCK = {
    row["sku"]: (row["on_hand"], row["reserved"])
    for row in catering_db.connection.execute("SELECT sku,on_hand,reserved FROM inventory")
}
print("Seeded stock:", SEEDED_STOCK, "| parent work:", catering_parent.id[:8])
CATERING_MESSAGES = [
    {
        "role": "system",
        "content": (
            "You run the counter of Lucy's ice cream shop. Answer catering inquiries with "
            "catering_quote, which is fixed arithmetic over the catalog: ten portions per tub, "
            "rounded up, draft only. When Lucy asks for a quote to be checked, assign exactly "
            "one research task with delegate_research, run the worker, and read its report. "
            "The shop's contract allows a research task at most eight model calls. Finish with "
            "a short plain-words summary: tubs, USD total, whether the worker verified the "
            "draft, and that nothing was reserved or purchased."
        ),
    },
    {"role": "user", "content": LUCY_REQUEST},
]
```

`catering_quote` takes the chapter's own `Inquiry`, so `guests` must be an integer from 1 to
200 and any extra field is refused as `invalid_arguments`. `delegate_research` extends it with
`model_calls`, bounded below by the schema and above by the runtime: the contract's one-to-eight
rule lives in `delegate`, not in the prompt and not in the Pydantic class, so a request for
twelve calls is a business refusal, `tool_failed`, with the reason in the ledger. The gate
refuses every write while the runtime is paused.

### Run the loop against a recorded transcript

The turns below are what a model replied, written down. Turn one quotes. Turn two asks for the
allowance Lucy suggested. Turn three asks for four. Turn four runs the worker and reads the
report in one turn. Turn five is the answer, which ends the loop. The transcript is built by
a function so the challenge can change one number without touching the story.

**Prediction:** how many tubs, what total in cents, and how many model calls does the worker
spend? `OfflineCateringModel` needs one call to ask for the quote and one to answer. Write
down what `research_report` returns for `status` and `model_calls` before you run it.


```python tags=["extension", "worked-example"]
def recorded_turns(model_calls):
    return [
        scripted_turn(
            calls=[{"name": "catering_quote", "arguments": {"sku": "SKU-VANILLA", "guests": 11}}]
        ),
        scripted_turn(
            calls=[
                {
                    "name": "delegate_research",
                    "arguments": {"sku": "SKU-VANILLA", "guests": 11, "model_calls": 12},
                }
            ]
        ),
        scripted_turn(
            calls=[
                {
                    "name": "delegate_research",
                    "arguments": {"sku": "SKU-VANILLA", "guests": 11, "model_calls": model_calls},
                }
            ]
        ),
        scripted_turn(
            calls=[
                {"name": "run_worker", "arguments": {}},
                {"name": "research_report", "arguments": {}},
            ]
        ),
        scripted_turn(
            "Eleven guests need 2 tubs of vanilla, USD 10.00 as a draft. Twelve model calls was "
            "over the contract, so the worker ran with four and verified the same draft. "
            "Nothing was reserved or purchased."
        ),
    ]


RECORDED_TURNS = recorded_turns(4)
LOOP_LIMITS = Limits(model_calls=8, tool_calls=16, seconds=120, output_tokens=4096)

scripted_result = run_loop(
    ScriptedModel(RECORDED_TURNS), catering_dispatcher, CATERING_MESSAGES, limits=LOOP_LIMITS
)
print(describe(scripted_result, "scripted"))
print("Refusal ledger:", catering_refusals)
scripted_trace = tool_trace(scripted_result)
assert scripted_result.status == "COMPLETED"
assert [step["ok"] for step in scripted_trace] == [True, False, True, True, True]
assert scripted_trace[0]["result"]["tubs"] == 2
assert scripted_trace[0]["result"]["total_cents"] == 1000
assert scripted_trace[0]["result"]["stock_reserved"] is False
assert scripted_trace[1]["result"] == "tool_failed"
assert catering_refusals == [("delegate_research", "bounded delegation contract required")]
assert scripted_trace[3]["result"]["status"] == "DONE"
assert scripted_trace[3]["result"]["quote"] == scripted_trace[0]["result"]
scripted_report = scripted_trace[4]["result"]
assert scripted_report["status"] == "DONE" and scripted_report["model_calls"] == 2
assert scripted_report["result"].startswith("Catering draft: 2 tubs for 11 guests, USD 10.00.")
# Durable state: one immutable assignment for the parent, and stock untouched.
assignments = catering_db.connection.execute(
    "SELECT count(*) FROM assistant_delegations WHERE parent_id=?", (catering_parent.id,)
).fetchone()[0]
...
The refused assignment is an ordinary failed observation; the loop carries on and the model
adapts. The child's row is real: `assistant_work` holds it with role `research`, `observe`
wrote its transcript, and `assistant_delegations.model_calls` counted the two calls the worker
spent against its limit of four. Stock did not move, because nothing in this path can move it.

### Run the same loop against a live model

Nothing changes except the model and a fresh desk. `build_model` looks for an `OPENAI_API_KEY`
in Colab's **Secrets** pane (the key icon in the left sidebar: add a secret with that name and
switch on notebook access for it), then in the process environment for a local kernel. Without
a key it returns the scripted model again and the record says so.

The live model is `gpt-5.1` through the `openai` library, which Colab ships preinstalled; on a
local kernel run `%pip install openai` once. The model may cap the allowance at eight on its
own, ask for twelve and adapt, or read the report before running the worker. What it cannot do
is create a second assignment for the same parent, widen the contract, or change stock.

**Prediction:** write down the tubs and total a careful assistant should report, and whether
it should accept Lucy's twelve, then run the cell and compare with the model's judgment, or
with the recorded transcript when the run is scripted.


```python tags=["extension", "live-model"]
from sovereign_agent.llm_lab import DEFAULT_MODEL, build_model, resolve_api_key

live_db, live_parent = open_catering_desk(LLM_CATERING_DB)
live_dispatcher, live_refusals = catering_desk(live_db, live_parent)
...
live_result = run_loop(chosen_model, live_dispatcher, CATERING_MESSAGES, limits=LOOP_LIMITS)
...
# Whatever the model did: every quote is the fixed arithmetic, every assignment is bounded.
for step in live_trace:
    if step["tool"] == "catering_quote" and step["ok"]:
        guests = step["arguments"]["guests"]
        assert step["result"]["tubs"] == (guests + 9) // 10, step
        assert step["result"]["total_cents"] == step["result"]["tubs"] * 500, step
        assert step["result"]["stock_reserved"] is False, step
    if step["tool"] == "delegate_research" and step["ok"]:
        assert 1 <= step["arguments"]["model_calls"] <= 8, step
live_children = live_db.connection.execute(
    "SELECT w.status,d.model_calls,d.model_calls_limit FROM assistant_delegations d "
    "JOIN assistant_work w ON w.id=d.work_id WHERE d.parent_id=?",
    (live_parent.id,),
).fetchall()
assert len(live_children) <= 1
for child in live_children:
    assert child["model_calls"] <= child["model_calls_limit"], dict(child)
live_stock = {
    row["sku"]: (row["on_hand"], row["reserved"])
    for row in live_db.connection.execute("SELECT sku,on_hand,reserved FROM inventory")
}
assert live_stock == SEEDED_STOCK
assert live_db.connection.execute("SELECT count(*) FROM assistant_orders").fetchone()[0] == 0
assert all(m.get("tool_call_id") for m in live_result.messages if m["role"] == "tool")
```

### Challenge: an allowance of one, then a paused runtime

The contract is data. `recorded_turns(1)` keeps every turn the same and changes only the
allowance the model settles on. The worker's loop admits one model call, `OfflineCateringModel`
asks for the quote with it, and the loop ends with `MODEL_CALL_LIMIT` before an answer exists.
`run_once` grades that as not passed and finishes the child as `BLOCKED`.

**Predict before running:** what does `research_report` return for `status` and `model_calls`,
and does `quote` called directly still say two tubs? The scripted answer still claims the
worker verified the draft; the ledger and the child's row say otherwise. Fixed arithmetic
earned the right answer with zero model calls; the delegation spent one and proved nothing.

Then pause the runtime with one `UPDATE assistant_control SET paused=1` and rerun the
transcript on the same desk: the gate refuses both assignments before `delegate` runs, and the
worker finds no claimable work. With a live key, rerun the live cell after either change and
explain whether the model noticed. In your notes, separate what the scripted run proves (the
contract, the grading, the gate, the rows) from what only a live run can show (the model's
choice of allowance). The saved record keeps the two apart.


```python tags=["extension", "retained-evidence"]
tight_db, tight_parent = open_catering_desk(LLM_CATERING_DB)
tight_dispatcher, tight_refusals = catering_desk(tight_db, tight_parent)
tight_result = run_loop(
    ScriptedModel(recorded_turns(1)), tight_dispatcher, CATERING_MESSAGES, limits=LOOP_LIMITS
)
print(describe(tight_result, "scripted"))
tight_trace = tool_trace(tight_result)
assert tight_result.status == "COMPLETED"
assert tight_trace[3]["result"]["status"] == "BLOCKED"
assert tight_trace[3]["result"]["passed"] is False
tight_report = tight_trace[4]["result"]
assert tight_report["status"] == "BLOCKED" and tight_report["model_calls"] == 1
assert quote(tight_db, Inquiry(sku="SKU-VANILLA", guests=11))["tubs"] == 2

with tight_db.immediate() as connection:
    connection.execute("UPDATE assistant_control SET paused=1 WHERE id=1")
paused_dispatcher, paused_refusals = catering_desk(tight_db, tight_parent)
paused_result = run_loop(
    ScriptedModel(RECORDED_TURNS), paused_dispatcher, CATERING_MESSAGES, limits=LOOP_LIMITS
)
paused_trace = tool_trace(paused_result)
print("Paused:", [step["ok"] for step in paused_trace], paused_refusals)
assert [step["ok"] for step in paused_trace] == [True, False, False, False, True]
assert paused_refusals == [
    ("delegate_research", "runtime paused"),
    ("delegate_research", "runtime paused"),
    ("run_worker", "runtime paused"),
]
...
llm_lab_report = {
    ...
    "scripted": {
        ...
        "refused": [list(entry) for entry in catering_refusals],
        "quote": scripted_trace[0]["result"],
        "research": scripted_report,
    },
    "session": {
        ...
        "research": [dict(child) for child in live_children],
        "answer": live_result.answer[:400],
    },
    "allowance_of_one": {"research": tight_report, "quote_tubs": 2},
    "paused": {"refused": [list(entry) for entry in paused_refusals]},
}
llm_report_path = COURSE_WORK / "ch17-a-llm-lab-report-v1.json"
...
for open_db in (catering_db, live_db, tight_db):
    open_db.close()
```
