...
jupyter:
  ...
  course:
    ...
    lesson_id: isolation
    planned_minutes: 90
    resource_id: profrod-sovereign-agent-ch14-a-tool-isolation-exercise
...
<!-- #region -->
# Chapter 14, Unit A: Construct tool isolation
...
[![Open in Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/profrodai/sovereign-agent/blob/main/book/exercises/ch14/profrod-sovereign-agent-ch14-a-tool-isolation-exercise.ipynb) Runs on Google Colab as it ships today, or on any local Python 3.12+ kernel.
...
2. Build dispatch from registry lookup through allowlist, strict arguments, write guard and bounded JSON result. Refuse before invoking an unauthorized handler and keep raw validation inputs out of errors.
...
You edit a complete function in a temporary copy of `src/sovereign_agent/tool_dispatch.py`. The real callers, database and tool boundaries remain connected. The self-contained setup above supplies the frozen development runtime. The notebook runs reviewed local subprocesses; it is not a security sandbox. No live account is required.
...
Inspect the supplied caller around the function in `src/sovereign_agent/tool_dispatch.py`. Draw the data path from the observed output through this function to its actual input or query. Then write your implementation from the contract.
...
LLM_SHOP_DB = COURSE_WORK / "ch14-a-llm-shop.sqlite"
...
def build_shop(*, allowed=MODEL_ALLOWED, before_write=None, max_result_bytes=16_384):
    ...
    def private_report(_: NoArguments):
        ...
        return {"cash_pence": cash, "note": "supplier terms stay with Lucy"}
...
llm_report_path = COURSE_WORK / "ch14-a-llm-lab-report-v1.json"
