...
jupyter:
  ...
  course:
    ...
    planned_minutes: 90
    resource_id: profrod-sovereign-agent-ch01-b-prompt-harness-repair-solution
...
def transfer_check(rows, claims):
    expected = {
        r["sku"]: max(0, r["target"] - r["on_hand"]) for r in rows if r["target"] > r["on_hand"]
    }
    return (
        isinstance(claims, dict)
        and set(claims) == set(expected)
        and all(type(claims[k]) is int and claims[k] == v for k, v in expected.items())
    )
```
...
assert accepted["estimated_cents"] == 3500
...
assert connected_hidden["draft"]["estimated_cents"] == 3500
