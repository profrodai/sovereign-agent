...
jupyter:
  ...
  course:
    ...
    planned_minutes: 90
    resource_id: profrod-sovereign-agent-ch01-b-misleading-model-comparison-exercise
...
[![Open in Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/profrodai/sovereign-agent/blob/main/book/exercises/ch01/profrod-sovereign-agent-ch01-b-misleading-model-comparison-exercise.ipynb) Runs on Google Colab as it ships today (Python 3.13), or on any local Python 3.12+ kernel. It needs nothing beyond Python's standard library.

This is the second of Chapter 1's two practical units. It starts from Unit A's tokenizer.

A colleague has compared tokenizers for Lucy's model and chosen the one with the lowest perplexity. The choice is wrong, and the reason is one of the most common mistakes in measuring language models: comparing per-token numbers across tokenizers, on training text. You will reproduce the mistake, build the fair measurement, and apply it to a real model's log-probabilities.

By the end you should be able to:

1. Define cross-entropy and perplexity, and explain why next-token training minimizes cross-entropy.
2. Explain why per-token perplexities of models with different tokenizers cannot be compared, and why training text cannot rank models.
3. Implement bits per character, and use it on held-out text to choose correctly.
4. Compute bits per character from a real model's natural-log log-probabilities.
...
def transfer_bits_per_character(log_probabilities, text):
    raise NotImplementedError("Convert nats to bits, then divide by characters")
```
