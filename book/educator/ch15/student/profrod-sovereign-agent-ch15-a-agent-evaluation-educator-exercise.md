...
[![Open in Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/profrodai/sovereign-agent/blob/main/book/exercises/ch15/profrod-sovereign-agent-ch15-a-agent-evaluation-exercise.ipynb) Runs on Google Colab as it ships today, or on any local Python 3.12+ kernel.
...
```python tags=["foundation", "worked-example"]
intro_case_results = [
    {"case": "ordinary", "status": "PASS", "cents": 2},
    {"case": "reserved", "status": "FAIL", "cents": 3},
    {"case": "provider-down", "status": "NOT_RUN", "cents": None},
]
...
print("Known recorded cost:", sum(row["cents"] for row in intro_observed))
...
## Extension: put a language model behind the evaluation report

Above, you ran `evaluate` yourself and read the report. The oracle, `baseline`, was written
without looking at `case.expected`, and the report's `passed` is the conjunction of named
checks over a transcript, never a judgment of the prose. The remaining question is what
happens when a model reads that report and is the one asked to record the verdict. A verdict
is durable: once Lucy's ledger says a case passed, the next chapter's activation boundary
trusts it.

This section makes the model an evaluation clerk. Its tools wrap the chapter's real functions:
`baseline` for the independent answer, `evaluate` with the runtime's `OfflineShopModel` as the
agent under test, and one consequential tool that appends a verdict to the append-only
`events` table through `append_event`. The loop is the Chapter 3 `run_loop`; the tools are
behind the Chapter 14 `Dispatcher`, so a verdict needs write authority and the guard can refuse.

The guard's rule is the chapter's rule: a verdict may only name a case that was evaluated in
this session, and it must say what the report said. `llm_lab.py` supplies `ScriptedModel` and
`OpenAIModel` behind the book's `Model` protocol; the record always says which one ran.

**Prediction:** the next cell prints four tool contracts. Write down which tool reveals the
authored `expected` answer, which one is consequential, and what `baseline` should return
for the `reserved_stock` case (physical 8, reserved 3, threshold 8) before you run it.
...
from reference_organizations.store.evaluation import CASES, Case, baseline, evaluate
...
LLM_VERDICT_DB = COURSE_WORK / "ch15-a-llm-verdicts.sqlite"
CASE_BOOK = {case.name: case for case in CASES}
...
class CaseArguments(NoArguments):
    name: str = Field(min_length=1, max_length=64)


class VerdictArguments(CaseArguments):
    passed: bool
    note: str = Field(min_length=1, max_length=200)


class VerdictGate:
    """Write authority for verdicts: only an evaluated case, and only what its report says."""

    def __init__(self):
        self.evaluated, self.refused = {}, []

    def __call__(self, call):
        name, passed = call.arguments.get("name"), call.arguments.get("passed")
        if name not in self.evaluated:
            reason = "verdict_without_evaluation"
        elif self.evaluated[name]["passed"] != passed:
            reason = "verdict_contradicts_evidence"
        else:
            return
        self.refused.append((name, reason))
        raise PermissionError(reason)


def fresh_verdict_database(path):
    """Start from an empty verdict ledger so reruns and replays observe the same events."""
...
def build_clerk(book=CASE_BOOK):
    """Four tools over the chapter's own functions; the case book never exposes `expected`."""
    db = fresh_verdict_database(LLM_VERDICT_DB)
    gate = VerdictGate()
    ...
    def list_cases(_: NoArguments):
        return [{"name": c.name, "split": c.split, "products": len(c.stock)} for c in book.values()]

    def describe_case(args: CaseArguments):
        return [
            {
                "sku": sku,
                "on_hand": on_hand,
            ...
            for sku, on_hand, reserved, point, price in case_for(args.name).stock
        ]

    def baseline_answer(args: CaseArguments):
        drafts = [
            {"sku": sku, "quantity": quantity} for sku, quantity in baseline(case_for(args.name))
        ]
        return {"drafts": drafts, "model_calls": 0}

    def evaluate_case(args: CaseArguments):
        report = evaluate(OfflineShopModel, cases=(case_for(args.name),), limits=EVALUATION_LIMITS)
        row = report["cases"][0]
        summary = {
            "case": row["case"],
            "passed": row["passed"],
            "checks": row["checks"],
            "observed": [list(item) for item in row["observed"]],
            "expected": [list(item) for item in row["expected"]],
            "acceptance": report["acceptance"]["status"],
        }
        gate.evaluated[args.name] = summary
        return summary

    def record_verdict(args: VerdictArguments):
        payload = {"case": args.name, "passed": args.passed, "note": args.note}
        with db.immediate():
            event = append_event(db, "evaluation_verdict", payload)
        return {"event": event.id, **payload}

    tools = [
        ExecutableTool(
            "list_cases", "List the authored scenario names and splits.", NoArguments, list_cases
        ),
        ExecutableTool(
            "describe_case",
            "Read one case's stock rows. Never shows the answer.",
            CaseArguments,
            describe_case,
        ),
        ExecutableTool(
            "baseline_answer",
            "The independent replenishment oracle for one case.",
            CaseArguments,
            baseline_answer,
        ),
        ExecutableTool(
            "evaluate_case",
            "Run the offline shop agent through the named checks.",
            CaseArguments,
            evaluate_case,
        ),
        ExecutableTool(
            "record_verdict",
            "Append a verdict to Lucy's ledger. Only for a case evaluated in this session.",
            VerdictArguments,
            record_verdict,
            consequential=True,
        ),
    ...
    dispatcher = Dispatcher(
        tools, allowed=frozenset(tool.name for tool in tools), before_write=gate
    )
    return db, dispatcher, gate


def verdicts(db):
    return [
        (e.payload["case"], e.payload["passed"])
        for e in replay(db)
        if e.kind == "evaluation_verdict"
    ]


clerk_db, clerk_dispatcher, clerk_gate = build_clerk()
for tool_schema in clerk_dispatcher.schemas():
    function = tool_schema["function"]
    print(function["name"], "requires", function["parameters"].get("required", []))
    print("   ", function["description"])
print("Consequential:", [t.name for t in clerk_dispatcher.tools.values() if t.consequential])
print("Cases:", {name: case.split for name, case in CASE_BOOK.items()})
print("Default limits:", EVALUATION_LIMITS)
CLERK_MESSAGES = [
    {
        "role": "system",
        "content": (
            "You are the evaluation clerk for Lucy's shop assistant. Evidence is what you produce "
            "in this session: read the case, ask for the independent baseline, run the "
            "evaluation, then record the verdict the report supports. Never record a verdict "
            "before evaluating and never one the report contradicts. If a call is refused, do "
            "not repeat it. Finish with a short summary that names the checks."
        ),
    },
    {"role": "user", "content": "Please check the reserved_stock case and record its verdict."},
]
```

`describe_case` shows stock rows and nothing else: the authored answer only appears in the
report after `evaluate_case` has run, alongside the observed drafts it was compared with.
`record_verdict` is the one consequential tool, and the guard attached to the dispatcher keeps
`evaluated`, the session's own evidence, next to `refused`, its ledger of reasons. The
dispatcher flattens a guard refusal to `tool_failed`, which is why the guard keeps that ledger.

### Run the loop against a recorded transcript

The turns below are what a model replied, written down. Turn one lists the cases. Turn two reads
`reserved_stock` and asks for the baseline. Turn three records a verdict on the strength of the
baseline alone. Turn four runs the evaluation. Turn five records the verdict again. Turn six
reports, which ends the loop.

**Prediction:** write the six observations in order as `ok` or refused, the reason the guard
logs for the refusal, the baseline's drafts, and how many `evaluation_verdict` events the
ledger holds when the loop stops. Then check each against the printed record.


```python tags=["extension", "worked-example"]
def recorded_turns(name):
    """The same six turns for any case: read, baseline, verdict too early, evaluate, verdict."""
    return [
        scripted_turn(calls=[{"name": "list_cases", "arguments": {}}]),
        scripted_turn(
            calls=[
                {"name": "describe_case", "arguments": {"name": name}},
                {"name": "baseline_answer", "arguments": {"name": name}},
            ]
        ),
        scripted_turn(
            calls=[
                {
                    "name": "record_verdict",
                    "arguments": {"name": name, "passed": True, "note": "baseline looks right"},
                }
            ]
        ),
        scripted_turn(calls=[{"name": "evaluate_case", "arguments": {"name": name}}]),
        scripted_turn(
            calls=[
                {
                    "name": "record_verdict",
                    "arguments": {"name": name, "passed": True, "note": "every named check passed"},
                }
            ]
        ),
        scripted_turn(
            f"Case {name}: the baseline and the offline agent agree, the evaluation passed its "
            "named checks and the verdict is recorded. My first verdict was refused because no "
            "evaluation had run yet."
        ),
    ]


RECORDED_TURNS = recorded_turns("reserved_stock")
LOOP_LIMITS = Limits(model_calls=8, tool_calls=16, seconds=120, output_tokens=4096)

scripted_db, scripted_dispatcher, scripted_gate = build_clerk()
scripted_result = run_loop(
    ScriptedModel(RECORDED_TURNS), scripted_dispatcher, CLERK_MESSAGES, limits=LOOP_LIMITS
)
print(describe(scripted_result, "scripted"))
print("Guard refused:", scripted_gate.refused, "| verdicts:", verdicts(scripted_db))
scripted_trace = tool_trace(scripted_result)
assert scripted_result.status == "COMPLETED"
assert [step["ok"] for step in scripted_trace] == [True, True, True, False, True, True]
assert scripted_trace[2]["result"]["drafts"] == [{"sku": "V", "quantity": 3}]
assert scripted_trace[3]["result"] == "tool_failed"
assert scripted_gate.refused == [("reserved_stock", "verdict_without_evaluation")]
evaluation = scripted_trace[4]["result"]
assert evaluation["passed"] is True and evaluation["observed"] == [["V", 3]]
assert evaluation["checks"]["baseline_matches_authored_answer"] is True
scripted_verdicts = verdicts(scripted_db)
assert scripted_verdicts == [("reserved_stock", True)]
...
The early verdict reached the transcript as a failed observation and the loop carried on;
the ledger holds one event, written after the evaluation. The baseline said three tubs, not
zero: physical minus reserved is five, three below the threshold. An oracle that forgot
`reserved` would have disagreed with the authored answer, and the report would have said so.
...
local kernel run `%pip install openai` once. It receives the same four schemas and the same
request. It may skip the baseline, evaluate more cases than asked, or try to record early. What
it cannot do is put a verdict in the ledger that its own session's evaluation does not support,
because that rule lives in the guard and not in the prompt.

**Prediction:** write which ledger properties must hold whatever the model does, then run the
cell and compare the model's judgment, or the recorded transcript, with your list.


```python tags=["extension", "live-model"]
from sovereign_agent.llm_lab import DEFAULT_MODEL, build_model, resolve_api_key

live_db, live_dispatcher, live_gate = build_clerk()
...
live_result = run_loop(chosen_model, live_dispatcher, CLERK_MESSAGES, limits=LOOP_LIMITS)
print(describe(live_result, chosen_model.source))
print("Guard refused:", live_gate.refused, "| verdicts:", verdicts(live_db))
live_trace = tool_trace(live_result)
live_verdicts = verdicts(live_db)
assert live_result.status in {"COMPLETED", "MODEL_CALL_LIMIT", "TOOL_LIMIT", "MODEL_FAILED"}
# Whatever the model did, every verdict in the ledger names a case this session evaluated and
# repeats that report's own outcome; every refused verdict has a reason in the guard's ledger.
for case_name, passed in live_verdicts:
    assert case_name in live_gate.evaluated and live_gate.evaluated[case_name]["passed"] == passed
refused_verdicts = [
    s for s in live_trace if s["tool"] == "record_verdict" and s["result"] == "tool_failed"
]
assert len(refused_verdicts) == len(live_gate.refused)
for step in live_trace:
    if step["tool"] == "evaluate_case" and step["ok"]:
        assert step["result"]["checks"]["baseline_matches_authored_answer"] is True, step
        assert step["result"]["checks"]["no_purchases"] is True, step
assert all(
    message.get("tool_call_id") for message in live_result.messages if message["role"] == "tool"
)
```

### Challenge: author a case, then author it wrongly

Cases are data, so add two. `saturday_hold` holds five of eight vanilla tubs back for a party,
with the expected answer calculated by hand from physical minus reserved. `saturday_hold_naive`
has the same stock but an authored answer written as if reservations did not exist. The recorded
transcript is reused unchanged for both; only the case name changes.

**Predict before running:** the baseline's drafts for the new case; which checks the naive
case fails and whether the offline agent or the authored answer is the one that is wrong; and
what the guard refuses at turn five on the naive case, given that the recorded model insists on
`passed: True`. The final direct call records the verdict the report actually supports.

In your notes, separate what the scripted runs prove (the oracle, the named checks, the guard,
the ledger) from what only a live run shows (whether the model read the report before writing).
A passing finite suite is still not proof of arbitrary language faithfulness; the saved record
keeps the scripted and live results apart so neither is mistaken for the other.


```python tags=["extension", "retained-evidence"]
SATURDAY_STOCK = (("SKU-VANILLA", 8, 5, 8, 250), ("SKU-CHOCOLATE", 12, 0, 6, 300))
AUTHORED_CASES = {
    **CASE_BOOK,
    "saturday_hold": Case("saturday_hold", "notebook", SATURDAY_STOCK, (("SKU-VANILLA", 5),)),
    "saturday_hold_naive": Case("saturday_hold_naive", "notebook", SATURDAY_STOCK, ()),
}
challenge_db, challenge_dispatcher, challenge_gate = build_clerk(AUTHORED_CASES)
hold_result = run_loop(
    ScriptedModel(recorded_turns("saturday_hold")),
    challenge_dispatcher,
    CLERK_MESSAGES,
    limits=LOOP_LIMITS,
)
hold_trace = tool_trace(hold_result)
print("saturday_hold:", [step["ok"] for step in hold_trace], hold_trace[2]["result"]["drafts"])
assert hold_trace[2]["result"]["drafts"] == [{"sku": "SKU-VANILLA", "quantity": 5}]
assert hold_trace[4]["result"]["passed"] is True and hold_trace[5]["ok"] is True

naive_result = run_loop(
    ScriptedModel(recorded_turns("saturday_hold_naive")),
    challenge_dispatcher,
    CLERK_MESSAGES,
    limits=LOOP_LIMITS,
)
naive_trace = tool_trace(naive_result)
naive_checks = naive_trace[4]["result"]["checks"]
print("saturday_hold_naive:", [step["ok"] for step in naive_trace])
print("  failed checks:", sorted(name for name, held in naive_checks.items() if not held))
print(
    "  observed:",
    naive_trace[4]["result"]["observed"],
    "authored:",
    naive_trace[4]["result"]["expected"],
)
assert naive_trace[4]["result"]["passed"] is False and naive_trace[5]["result"] == "tool_failed"
assert (
    naive_checks["quantities"] is False
    and naive_checks["baseline_matches_authored_answer"] is False
)
assert naive_trace[4]["result"]["observed"] == [["SKU-VANILLA", 5]]
assert challenge_gate.refused[-1] == ("saturday_hold_naive", "verdict_contradicts_evidence")

supported = challenge_dispatcher.invoke(
    ToolCall(
        id="supported",
        name="record_verdict",
        arguments={
            "name": "saturday_hold_naive",
            "passed": False,
            "note": "authored answer ignores reserved",
        },
    )
)
print("Supported verdict:", supported["ok"], "| ledger:", verdicts(challenge_db))
assert supported["ok"] is True
assert verdicts(challenge_db) == [("saturday_hold", True), ("saturday_hold_naive", False)]
...
llm_lab_report = {
    ...
    "scripted": {
        ...
        "refused": scripted_gate.refused,
        "verdicts": scripted_verdicts,
    },
    "session": {
        ...
        "evaluated": sorted(live_gate.evaluated),
        "refused": live_gate.refused,
        "verdicts": live_verdicts,
        "answer": live_result.answer[:400],
    },
    "authored": {
        "saturday_hold": {"baseline": hold_trace[2]["result"]["drafts"], "passed": True},
        "saturday_hold_naive": {
            "failed_checks": sorted(name for name, held in naive_checks.items() if not held),
            "refused": challenge_gate.refused[-1],
            "verdicts": verdicts(challenge_db),
        },
    },
}
llm_report_path = COURSE_WORK / "ch15-a-llm-lab-report-v1.json"
