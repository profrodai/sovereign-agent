...
jupyter:
  ...
  course:
    ...
    planned_minutes: 90
    resource_id: profrod-sovereign-agent-ch15-a-agent-evaluation-solution
...
def transfer_check(rows):
    return {
        r["sku"]: r["target"] - r["physical"] + r["reserved"]
        for r in rows
        if r["target"] > r["physical"] - r["reserved"]
    }
```
...
Expected values are authored before candidate execution. Removing the expected-answer field from the candidate interface makes that particular shortcut unavailable, while changed identities challenge fixture lookup.
...
INSTRUCTOR_TRANSFER_CASES = [
    ("overreserved", [[{"sku": "NEW", "physical": 1, "reserved": 9, "target": 3}]], {"NEW": 11}),
...
# Instructor holdout appended to a submitted Chapter 12 Unit A.
...
try:
    ...
    outcome = task.transfer(ROOT / "book/always_on/exercises/ch12/holdouts/runtime-transfer-v1.py")
    assert outcome["status"] == "PASS", outcome
finally:
    task.close()
print("HOLDOUT_RESULT=" + json.dumps({"unit": "ch15-a", "status": "PASSED"}, sort_keys=True))
```
