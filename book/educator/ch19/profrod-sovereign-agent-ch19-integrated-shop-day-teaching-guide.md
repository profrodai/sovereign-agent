# Chapter 19: teach the mechanism, then test transfer
...
Construct the operating report from one read snapshot: independently query work, orders, ledger, stock, usage and delivery uncertainty; report disagreement and unknowns; preserve honest scope and bounded detail. Create a real approval, corrupt a retained ledger amount, then call the actual report and inspect both structured evidence and displayed exception text.
...
The additional task is **Reconcile an independently supplied order ledger**:
...
**Surviving limitation:** Local consistency is not an external account audit, daily cash profit or classroom learning evidence.
