...
Trace model admission, actual tool observations and terminal counters; charge failed admitted attempts and refuse repeated call identities.

Unit A: **Build a bounded model and tool loop**. Unit B: **Repair failed-call accounting**. Allocate ninety minutes to each,
...
**A provider failure does not erase an admitted attempt or its configured exposure.**
...
The additional task is **Admit a variable-cost next model attempt**:
...
**Surviving limitation:** An authored replay demonstrates loop behavior, not the frequency of a live model choosing that sequence.
