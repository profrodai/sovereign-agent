...
jupyter:
  ...
  course:
    ...
    planned_minutes: 90
    resource_id: profrod-sovereign-agent-ch03-b-reliability-and-retries-exercise
...
[![Open in Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/profrodai/sovereign-agent/blob/main/book/exercises/ch03/profrod-sovereign-agent-ch03-b-reliability-and-retries-exercise.ipynb) Runs on Google Colab as it ships today (Python 3.13), or on any local Python 3.12+ kernel. It needs nothing beyond Python's standard library, and it makes no network call.

This is the second of Chapter 3's two practical units. Unit A built a bounded loop. This unit asks how often a loop like it succeeds.

A colleague has estimated the reliability of Lucy's agent. Each step works 95% of the time, they say, so a 20-step task succeeds $0.95^{20} \approx 36\%$ of the time, and three attempts raise that to about 74%. Both numbers rest on an assumption the loop does not satisfy. You will derive the right formula, test it against a simulated agent whose true error and recovery rates you know, and then test the retry claim on data.

By the end you should be able to:

1. Derive the success probability of a loop that can recover from errors, as a two-state Markov chain.
2. Test a formula against simulation with a z-score, and show with a negative control that the test can fail.
3. Explain why retries of the same task are not independent, and measure it from retry data.
4. Say which reliability numbers are properties of the model, and which are properties of the loop around it.
...
def retry_after_failure(outcomes):
    raise NotImplementedError("Condition on a failed first attempt, then read the second")
```
