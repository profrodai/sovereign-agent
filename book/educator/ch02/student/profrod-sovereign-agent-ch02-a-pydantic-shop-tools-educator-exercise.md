...
[![Open in Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/profrodai/sovereign-agent/blob/main/book/exercises/ch02/profrod-sovereign-agent-ch02-a-pydantic-shop-tools-exercise.ipynb) Runs on Google Colab as it ships today, or on any local Python 3.12+ kernel.
...
2. Construct the complete tool factory: reject duplicate product identities, isolate a copied stock snapshot, expose stock and supplier tools, and make draft quantity equal current need. Preserve strict Pydantic argument validation and USD integer-cents arithmetic.
...
Work with integer cents so multiplication does not introduce a floating-point rounding question.
...
```python tags=["foundation", "worked-example"]
intro_shop = [
    {"sku": "MANGO", "on_hand": 1, "target": 5, "price_cents": 325},
    {"sku": "COCOA", "on_hand": 9, "target": 6, "price_cents": 300},
]
for intro_product in intro_shop:
    intro_needed = max(0, intro_product["target"] - intro_product["on_hand"])
    intro_total = intro_needed * intro_product["price_cents"]
    print(intro_product["sku"], intro_needed, intro_total)
assert 4 * 325 == 1300
```

Mango needs four tubs, costing 1300 cents. Cocoa needs none. A draft requesting three mango tubs
...
Construct the complete tool factory: reject duplicate product identities, isolate a copied stock snapshot, expose stock and supplier tools, and make draft quantity equal current need. Preserve strict Pydantic argument validation and USD integer-cents arithmetic.
...
## Extension: put a language model behind the typed tools

Every tool call above was written by you, one line at a time. The reason the shop tools have
a schema, a business rule and a `Dispatcher` in front of them is that in production the caller
is not you: it is a model, and it can ask for anything. This closing section puts a real model
behind the same three tools and shows how far the boundaries hold.

The mechanism is the loop you will build in Chapter 3, shipped in the embedded runtime as
`agent_loop.run_loop`. It sends the conversation and the tool schemas to a model, reads back
either an answer or tool calls, runs each call through the dispatcher, appends the observation
under the same `tool_call_id`, and asks again, while counting every attempt against `Limits`.
The model only ever *proposes* a call. The dispatcher checks the arguments against the Pydantic
class; the handler applies the business rule; nothing the model says can purchase anything.

The runtime adds one file for this section, `llm_lab.py`. It implements the loop's `Model`
protocol twice: `ScriptedModel` replays turns written down in the notebook, so this section
runs identically for everyone, and `OpenAIModel` sends the same messages to a hosted model
when a key is present. The loop cannot tell them apart; the record always says which one ran.

**Prediction:** the next cell builds the dispatcher from the supplied reference `build_tools`
(not your Unit A copy, which the harness closed) and prints what the model will see. Write down
which tool has no arguments, which argument is an integer, and what the loop's default limits
are before you run it.
...
# The learner module resolves the Chapter 1 checkpoint relative to the runtime root.
with contextlib.chdir(COURSE_ROOT):
    chapter_two = runpy.run_path(str(COURSE_ROOT / "book/always_on/learner/ch02.py"))
shop_dispatcher = chapter_two["build_tools"](chapter_two["SHOP"])
shop_need = {
    row["sku"]: max(0, row["reorder_point"] - row["on_hand"])
    for row in chapter_two["SHOP"]["products"]
}
for tool_schema in shop_dispatcher.schemas():
    function = tool_schema["function"]
    print(function["name"], "requires", function["parameters"].get("required", []))
    print("   ", function["description"])
print("Need per product:", shop_need)
...
SHOP_MESSAGES = [
    {
        ...
        "content": (
            ...
            "the shop needs. Finish with a short plain-words summary of what you drafted."
        ),
    },
    {"role": "user", "content": "What needs ordering this morning?"},
]
```

The schemas are the same `model_json_schema()` output you inspected earlier, wrapped in the
`{"type": "function", ...}` envelope the API expects; `required` comes from the fields without
defaults. `Limits()` is the budget the loop enforces before every attempt: model calls, tool
calls, seconds, context bytes and output tokens. Chapter 3 makes you construct that admission
decision; here you watch it work.
...
asks for stock. Turn two asks for a supplier price and two drafts, one of which asks for the
wrong quantity. Turn three corrects that draft. Turn four gives the answer, which ends the loop.

**Prediction:** which draft is refused, and what does the model see when it is refused? Recall
that `Dispatcher.invoke` flattens every handler exception to `tool_failed` so a raw error can
never leak an argument; the model has to work out the right quantity from `list_stock` itself.
...
RECORDED_TURNS = [
    ...
    scripted_turn(
        calls=[
            ...
            {"name": "draft_order", "arguments": {"sku": "SKU-STRAWBERRY", "quantity": 3}},
        ]
    ),
    scripted_turn(
...
assert [step["ok"] for step in scripted_trace] == [True, True, True, False, True]
assert scripted_trace[3]["result"] == "tool_failed"
assert scripted_trace[4]["result"]["total_cents"] == 4 * 275
...
The refused draft appears in the transcript as an ordinary failed observation, and the loop
carries on: refusing is not crashing. Count the tool calls in the record against the limits you
predicted, and find the observation whose `tool_call_id` matches the refused request.
...
receives the same three schemas and the same system message. It may draft in a different
order, ask the supplier first, or skip a product; what it cannot do is draft a quantity the
shop does not need, because that rule lives in the handler and not in the prompt.

**Prediction:** write the drafts a careful assistant should produce from the stock you printed
above. Then run the cell and compare the model's judgment, or the recorded transcript when the
run is scripted, against your list.
...
live_result = run_loop(chosen_model, shop_dispatcher, SHOP_MESSAGES, limits=LOOP_LIMITS)
...
# Whatever the model did, every draft that succeeded matches the shop's own need.
...
### Challenge: change the budget, then change the shop

The loop's admission rule is data. Rerun the recorded transcript with `Limits(tool_calls=2)` and
predict, before running, which status ends the loop and how many observations the transcript
holds when it stops. Then change one product's `on_hand` in a copy of `SHOP`, rebuild the
dispatcher, and show that the recorded transcript's corrected strawberry draft is now refused
too: the transcript did not change, the shop did. With a live key, rerun the live cell after the
change and explain whether the model recovered by reading stock again.

In your notes, separate what the scripted run proves (the loop, the dispatcher, the handler
rule, the transcript accounting) from what only a live run can show (the model's judgment).
The saved record keeps the two apart.
...
tight_result = run_loop(
    ...
    shop_dispatcher,
    SHOP_MESSAGES,
    limits=Limits(model_calls=8, tool_calls=2, seconds=120, output_tokens=4096),
)
print("Tight budget:", tight_result.status, "after", tight_result.tool_calls, "tool calls")
assert tight_result.status == "TOOL_LIMIT" and tight_result.tool_calls == 1
...
llm_lab_report = {
    ...
    "session": {
        ...
        "drafted": [
            step["arguments"] for step in live_trace if step["tool"] == "draft_order" and step["ok"]
        ],
        "answer": live_result.answer[:400],
    },
    "tight_budget": {"limits": {"tool_calls": 2}, "status": tight_result.status},
}
llm_report_path = COURSE_WORK / "ch02-a-llm-lab-report-v1.json"
