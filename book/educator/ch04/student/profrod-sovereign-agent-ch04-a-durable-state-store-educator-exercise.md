...
jupyter:
  ...
  course:
    ...
    planned_minutes: 90
    resource_id: profrod-sovereign-agent-ch04-a-durable-state-store-exercise
...
[![Open in Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/profrodai/sovereign-agent/blob/main/book/exercises/ch04/profrod-sovereign-agent-ch04-a-durable-state-store-exercise.ipynb) Runs on Google Colab as it ships today (Python 3.13), or on any local Python 3.12+ kernel. It needs nothing beyond Python's standard library.

This is one of two practical units for Chapter 4. Unit A constructs the store and connects it to a day of Lucy's shop; Unit B reproduces an interrupted schema change, repairs it and transfers the rule. Each is a complete ninety-minute session. Basic Python variables, conditions, loops, functions, lists and dictionaries are the starting knowledge. SQLite, transactions and context managers are introduced below before the main task.

By the end you should be able to:

1. Predict what a second connection sees after a program stops between two writes, and check the prediction.
2. Explain a table, a constraint, a parameter and a transaction using Lucy's stock.
3. Implement `apply_event`, which records an event and its stock change in one transaction, treats a replay as a duplicate and refuses a reused identity.
4. State the invariant stock = sum of events, and use an independent reader to check it.
...
These are planning estimates, not measured completion times. Run All checks that the notebook executes; unfinished student functions deliberately produce NEEDS_WORK. Keep your first attempt before opening answers.
...
def apply_event(connection, event, between=None):
    ...
    connection.execute(
        "INSERT INTO events VALUES (?, ?, ?)", (event["event_id"], event["sku"], event["delta"])
    )
    ...
    if updated == 0:
        connection.execute("INSERT INTO stock VALUES (?, ?)", (event["sku"], event["delta"]))
    return "applied"
```
...
def transfer_check(existing, event):
    raise NotImplementedError("Decide the append before writing anything")
```
