...
[![Open in Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/profrodai/sovereign-agent/blob/main/book/exercises/ch10/profrod-sovereign-agent-ch10-b-approval-repair-transfer-exercise.ipynb) Runs on Google Colab as it ships today, or on any local Python 3.12+ kernel.
...
intro_proposal = {"sku": "MANGO", "quantity": 4, "unit_cents": 325}
intro_reordered = {"unit_cents": 325, "quantity": 4, "sku": "MANGO"}
...
proposal uses a validated schema and integer cents; hashing arbitrary malformed input would not
...
authority is the ceiling minus both. If 700 cents is spent and 900 reserved under a 2000-cents
ceiling, a 500-cents order is too large even though 500 is smaller than 2000.
...
Write a table for a fresh draft, repeated approval, changed digest, one-cents overflow, unknown
...
Test exact-limit acceptance, one-cents overflow, changed digest, stricter policy and repeated approval. Never call a real supplier.
...
## Extension: put a language model behind exact approval

Every `propose` and `approve` above was called by you, with a digest you copied by hand. In
production the caller who proposes replenishment is a model, and the approval mechanism is
the boundary that keeps money honest: an approval names one exact digest, reserves that
amount once, and compares spent plus reserved plus the new amount against the smaller of the
installed and supplied ceilings. This closing section puts a real model behind that boundary.

The mechanism is the Chapter 3 loop, `agent_loop.run_loop`: the model proposes a call, the
dispatcher checks it against a Pydantic class, a `before_write` gate may refuse it, and the
handler calls the same `propose` and `approve` from `assistant_orders.py` that you traced.
Lucy has granted a standing automatic allowance of 1500 cents per order under a 2000 cents
total, so the model may invoke `approve` with `automatic=True` and the record says
`AUTOMATIC`. Beyond either number only Lucy's own `/approve` can act, and there is no tool for
that. Nothing here sends an order to a supplier: an approval is bounded authority, not a receipt.
...
APPROVAL_DB = COURSE_WORK / "ch10-llm-approval.sqlite"
TARGET = "lucy-local"
OPERATOR = "lucy"
SHOP_POLICY = SpendingPolicy(frozenset({OPERATOR}), total_cents=2000, automatic_order_cents=1500)


class ApprovalArguments(StockArguments):
    digest: str = Field(min_length=64, max_length=64, pattern=r"^[0-9a-f]{64}$")


def exact_digest(sku, quantity, unit_cost_cents, target=TARGET):
    """The same bytes `propose` hashes: the digest is a function of the exact proposal."""
    proposal = {
        "sku": sku,
        "quantity": quantity,
...
def open_shop(path):
    """A fresh seeded ledger with one claimed work item, so reruns observe the same rows."""
    for stale in (path, Path(f"{path}-wal"), Path(f"{path}-shm"), path.with_suffix(".authority")):
        stale.unlink(missing_ok=True)
    db = Database(path)
    seed_lucy(db)
    enqueue(db, "llm-lab:morning", "lucy", "Replenish low stock")
    work = claim(db, "notebook-worker", ttl=900)
    assert work is not None
    return db, work


def spending_row(db):
    row = db.connection.execute("SELECT * FROM assistant_spending WHERE id=1").fetchone()
    if row is None:
        return {"limit_cents": None, "reserved_cents": 0, "spent_cents": 0}
    return {k: row[k] for k in ("limit_cents", "reserved_cents", "spent_cents")}


def order_rows(db, work):
    rows = db.connection.execute(
        "SELECT json_extract(proposal,'$.sku') AS sku,amount,status,approval_basis,approved_by "
        "FROM assistant_orders WHERE work_id=? ORDER BY created,id",
        (work.id,),
    )
    return [dict(row) for row in rows]


def build_approval_tools(db, work, policy):
    """Two reading tools from the shop, two writing tools over the chapter's mechanism."""
    refusals = []
    reads = shop_dispatcher(db)

    def live_order(sku):
        return db.connection.execute(
            "SELECT id FROM assistant_orders WHERE work_id=? AND json_extract(proposal,'$.sku')=? "
            "AND status IN ('DRAFT','APPROVED') AND revoked=0",
            (work.id, sku),
        ).fetchone()

    def propose_order(args):
        checked = reads.invoke(
            ToolCall(id="validate-draft", name="draft_order", arguments=args.model_dump())
        )
        if not checked["ok"]:
            refusals.append(("propose_order", "proposal must match the current stock need"))
            raise ValueError("proposal must match the current stock need")
        try:
            identifier = propose(db, work, args.sku, args.quantity, target=TARGET)
        except (ValueError, PermissionError) as error:
            refusals.append(("propose_order", str(error)))
            raise
        row = db.connection.execute(
            "SELECT digest,amount,status FROM assistant_orders WHERE id=?", (identifier,)
        ).fetchone()
        return {"sku": args.sku, "quantity": args.quantity, **dict(row), "currency": "USD"}

    def approve_order(args):
        order = live_order(args.sku)
        try:
            approve(
                db,
                order["id"],
            ...
            refusals.append(("approve_order", str(error)))
            raise
        return {"sku": args.sku, "status": "APPROVED", **spending_row(db)}

    def spending(_):
        return {**spending_row(db), "orders": order_rows(db, work)}

    def before_write(call):
        # Approval is scoped to proposals this work item made; other work is not the model's.
        if call.name == "approve_order" and live_order(call.arguments.get("sku")) is None:
            refusals.append(("approve_order", "no live proposal for that product in this work"))
            raise PermissionError("no live proposal for that product in this work")

    tools = [
        reads.tools["list_stock"],
        reads.tools["supplier"],
        ExecutableTool(
            "propose_order",
            "Record an exact proposal for the quantity the shop needs. Never purchases.",
            DraftArguments,
            propose_order,
            consequential=True,
        ),
        ExecutableTool(
            "approve_order",
            "Approve one exact proposal by its digest under Lucy's automatic allowance.",
            ApprovalArguments,
            approve_order,
            consequential=True,
        ),
        ExecutableTool(
            "spending",
            "Read the spending ledger and every order of this work item.",
            NoArguments,
            spending,
        ),
    ]
...
approval_db, approval_work = open_shop(APPROVAL_DB)
approval_dispatcher, approval_refusals = build_approval_tools(
    approval_db, approval_work, SHOP_POLICY
)
for tool_schema in approval_dispatcher.schemas():
    function = tool_schema["function"]
    print(function["name"], "requires", function["parameters"].get("required", []))
    print("   ", function["description"])
print("Policy:", SHOP_POLICY)
VANILLA_DIGEST = exact_digest("SKU-VANILLA", 6, 250)
STRAWBERRY_DIGEST = exact_digest("SKU-STRAWBERRY", 4, 275)
print("Digest of the exact vanilla proposal:", VANILLA_DIGEST)
APPROVAL_MESSAGES = [
    {
        "role": "system",
        "content": (
            "You help Lucy run her ice cream shop. Read stock with the tools before proposing "
            "anything. Propose exactly the quantity the shop needs. You may approve a proposal "
            "under Lucy's automatic allowance by quoting the digest the proposal returned; if "
            "the shop refuses, leave that proposal as a draft for Lucy and say so. Finish with "
            "a short plain-words summary of what is approved, what is reserved and what waits."
        ),
    },
    {"role": "user", "content": "Vanilla and strawberry are low. Sort out the replenishment."},
]
```

The two reading tools are the shop's own `list_stock` and `supplier` from the embedded
runtime; `list_stock` already counts approved orders as `on_order`, so an approval changes what
the next read says is needed. `propose_order` reuses the shop's `draft_order` rule before it
calls `propose`, so a quantity that is not the current need never reaches the ledger. The
digest printed above was computed in the notebook from the proposal bytes alone, which is
the whole point of exact approval: anyone holding the proposal can name it.
...
RECORDED_TURNS = [
    ...
    scripted_turn(
        calls=[
            {
                "name": "approve_order",
                "arguments": {"sku": "SKU-VANILLA", "digest": VANILLA_DIGEST},
            },
            {
                "name": "approve_order",
                "arguments": {"sku": "SKU-STRAWBERRY", "digest": STRAWBERRY_DIGEST},
            },
        ]
    ),
...
scripted_result = run_loop(
    ScriptedModel(RECORDED_TURNS), approval_dispatcher, APPROVAL_MESSAGES, limits=LOOP_LIMITS
)
print(describe(scripted_result, "scripted"))
scripted_trace = tool_trace(scripted_result)
scripted_spending = spending_row(approval_db)
scripted_orders = order_rows(approval_db, approval_work)
print("Refusals:", approval_refusals)
print("Spending:", scripted_spending)
...
assert approval_refusals == [("approve_order", "cumulative spending ceiling reached")]
# The digest the ledger minted is the one the notebook computed before the run.
assert scripted_trace[1]["result"]["digest"] == VANILLA_DIGEST
assert scripted_trace[1]["result"]["amount"] == 6 * 250
assert scripted_spending == {"limit_cents": 2000, "reserved_cents": 1500, "spent_cents": 0}
assert [(o["sku"], o["status"], o["approval_basis"]) for o in scripted_orders] == [
    ("SKU-VANILLA", "APPROVED", "AUTOMATIC"),
    ("SKU-STRAWBERRY", "DRAFT", "UNKNOWN"),
]
...
The strawberry approval fits the per-order allowance and still fails, because the ceiling
is cumulative: 1500 already reserved plus 1100 is 2600, above 2000. The refusal reached the
model as a plain `tool_failed`; the reason lives only in the notebook's ledger, since `invoke`
never copies an error message into an observation. The transaction left the ledger unchanged:
reserved stayed at 1500 and the strawberry row stayed a draft, which is what Lucy will see.
...
receives the same five schemas and system message, on a fresh ledger. It may approve strawberry
first and leave vanilla waiting, or approve nothing; what it cannot do is reserve more than the
ceiling, reserve one proposal twice, or approve another work item's order, because those rules
live in `approve` and in the gate.

**Prediction:** write which order a careful assistant should approve when only one fits, then
run the cell and compare the model's judgment, or the recorded transcript when scripted.
...
approval_db.close()
live_db, live_work = open_shop(APPROVAL_DB)
live_dispatcher, live_refusals = build_approval_tools(live_db, live_work, SHOP_POLICY)
...
live_result = run_loop(chosen_model, live_dispatcher, APPROVAL_MESSAGES, limits=LOOP_LIMITS)
print(describe(live_result, chosen_model.source))
print("Refusals:", live_refusals)
live_trace = tool_trace(live_result)
live_spending = spending_row(live_db)
live_orders = order_rows(live_db, live_work)
print("Spending:", live_spending)
for order in live_orders:
    print(order)
assert live_result.status in {"COMPLETED", "MODEL_CALL_LIMIT", "TOOL_LIMIT", "MODEL_FAILED"}
# Whatever the model did, the ledger obeys the chapter's arithmetic and authority rules.
approved = [order for order in live_orders if order["status"] == "APPROVED"]
assert live_spending["reserved_cents"] == sum(order["amount"] for order in approved)
assert live_spending["reserved_cents"] + live_spending["spent_cents"] <= SHOP_POLICY.total_cents
assert live_spending["spent_cents"] == 0  # nothing was ever sent to a supplier
for order in approved:
    assert order["amount"] <= SHOP_POLICY.automatic_order_cents
    assert (order["approval_basis"], order["approved_by"]) == ("AUTOMATIC", OPERATOR)
assert all(order["status"] in {"DRAFT", "APPROVED", "REVOKED"} for order in live_orders)
UNIT_COST = {"SKU-VANILLA": 250, "SKU-CHOCOLATE": 300, "SKU-STRAWBERRY": 275}
for step in live_trace:
    if step["tool"] == "propose_order" and step["ok"]:
        sku, quantity = step["arguments"]["sku"], step["arguments"]["quantity"]
        assert step["result"]["amount"] == quantity * UNIT_COST[sku], step
```

### Challenge: the exact limit, one cents over, and a stricter supplied policy

The ceiling is data. Rerun the recorded transcript on a fresh ledger with
`total_cents=2600`, which is exactly the two drafts together, and predict before running
...
from what only a live run can show (the model's judgment). The saved record keeps the two apart.


```python tags=["extension", "retained-evidence"]
def rerun_with(total_cents, name):
    db, work = open_shop(COURSE_WORK / f"ch10-llm-approval-{name}.sqlite")
    policy = SpendingPolicy(
        frozenset({OPERATOR}), total_cents=total_cents, automatic_order_cents=1500
    )
    dispatcher, refusals = build_approval_tools(db, work, policy)
    result = run_loop(
        ScriptedModel(RECORDED_TURNS), dispatcher, APPROVAL_MESSAGES, limits=LOOP_LIMITS
    )
    print(f"total_cents={total_cents}:", result.status, spending_row(db), "refusals:", refusals)
    return db, work, policy, refusals


exact_db, exact_work, exact_policy, exact_refusals = rerun_with(2600, "exact")
assert exact_refusals == [] and spending_row(exact_db)["reserved_cents"] == 2600
over_db, _, _, over_refusals = rerun_with(2599, "over")
assert over_refusals == [("approve_order", "cumulative spending ceiling reached")]
assert spending_row(over_db)["reserved_cents"] == 1500

vanilla_id = exact_db.connection.execute(
    "SELECT id FROM assistant_orders WHERE json_extract(proposal,'$.sku')='SKU-VANILLA'"
).fetchone()[0]
renewal = time.time() + 60
approve(exact_db, vanilla_id, VANILLA_DIGEST, actor=OPERATOR, policy=exact_policy, expires=renewal)
assert spending_row(exact_db)["reserved_cents"] == 2600  # reapproval reserved nothing new
stricter = SpendingPolicy(frozenset({OPERATOR}), total_cents=2000)
try:
    approve(exact_db, vanilla_id, VANILLA_DIGEST, actor=OPERATOR, policy=stricter, expires=renewal)
except PermissionError as error:
    stricter_refusal = str(error)
else:
    stricter_refusal = None
print("Stricter supplied policy:", stricter_refusal)
assert stricter_refusal == "cumulative spending ceiling reached"
assert spending_row(exact_db)["reserved_cents"] == 2600
...
llm_lab_report = {
    ...
    "scripted": {
        ...
        "refused": [reason for _, reason in approval_refusals],
        "spending": scripted_spending,
        "orders": [(o["sku"], o["status"]) for o in scripted_orders],
    },
    "session": {
        ...
        "spending": live_spending,
        "orders": [(o["sku"], o["status"], o["approval_basis"]) for o in live_orders],
        "answer": live_result.answer[:400],
    },
    "ceilings": {
        "2600": {"refused": [r for _, r in exact_refusals], "reserved": 2600},
        "2599": {"refused": [r for _, r in over_refusals], "reserved": 1500},
        "reapproval_under_2000": stricter_refusal,
    },
}
llm_report_path = COURSE_WORK / "ch10-b-llm-lab-report-v1.json"
...
for open_db in (live_db, exact_db, over_db):
    open_db.close()
```
