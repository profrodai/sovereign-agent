# Chapter 10: teach the mechanism, then test transfer
...
Construct approval of one exact digest: validate operator, expiry, current work and proposal state; preserve uncertain outcomes; reserve only a new draft amount; enforce both installed and supplied account ceilings. Propose real orders, approve them and inspect retained reservations. Reapproval must not reserve twice and refusal must leave the transaction unchanged.

Unit A: **Construct exact approval**. Unit B: **Break, repair and transfer exact approval**. Allocate ninety minutes to each,
...
The additional task is **Apply the stricter cumulative spending ceiling**:
...
**Surviving limitation:** An approval is bounded authority, not a supplier receipt.
