...
def regression_suite(db):
    ...
    def evaluate_candidate(skill):
        ...
        checks["instructions_name_gbp"] = "USD" in skill.instructions
        return checks
...
def skill_desk(db, evaluator):
    ...
    def before_write(call):
        if call.arguments.get("name") not in SKILL_NAMES:
            refusals.append((call.name, call.arguments.get("name"), None, "name not in catalog"))
            raise PermissionError("name not in catalog")
...
SKILL_MESSAGES = [
    {
        ...
        "content": (
            ...
            "passes. Lucy's rule: any procedure that mentions prices must say USD. Read the "
    ...
    },
    {
        ...
        "content": (
            ...
            "If the shop refuses it, revise the wording to say USD and stage it as version 2."
        ),
    },
]
```
...
is written. The `before_write` gate adds Lucy's own rule on top: only names in her catalog
may be staged or activated. A schema refusal and a catalog refusal are different boundaries
...
VERSION_TWO = "Greet the customer, check stock before promising anything, and quote prices in USD."
RECORDED_TURNS = [
    ...
    scripted_turn(
        ...
        "Version 2 says USD, passed the regression suite and is now the active opening procedure."
    ),
]
...
before activating either, or try a name outside the catalog. What it cannot do is activate a
...
# Whatever the model did, every active skill passed the suite and sits inside the catalog.
...
for reporting_version in ("1", "2"):
    ...
    reporting_path.write_text(
        ...
        'instructions = "Report totals in USD."\nrequires = []\n',
        encoding="utf-8",
    )
