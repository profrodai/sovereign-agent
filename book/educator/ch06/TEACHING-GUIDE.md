...
Construct context assembly from active eligible skills, explicit preferences and revision-matching completed history. Enforce the byte budget and retain provenance before adding the current user request. Stage and activate a local skill through the existing API, then inspect context with and without its required tool capability.

Unit A: **Construct versioned skills**. Unit B: **Break, repair and transfer versioned skills**. Allocate ninety minutes to each,
...
The additional task is **Admit only complete skill requirements**:
...
**Surviving limitation:** Eligibility and fixture activation checks do not prove live model quality.
