...
jupyter:
  ...
  course:
    ...
    lesson_id: skills
    planned_minutes: 90
    resource_id: profrod-sovereign-agent-ch06-b-skill-repair-transfer-solution
...
def transfer_check(skills, allowed):
    return [s["name"] for s in skills if s["active"] and set(s["requires"]) <= set(allowed)]
```
...
Intersection answers whether at least one capability overlaps. Subset answers whether every requirement is available. Neither parsing nor active status replaces that complete requirement check.
...
try:
    ...
    task.close()
print("HOLDOUT_RESULT=" + json.dumps({"unit": "ch06-b", "status": "PASSED"}, sort_keys=True))
```
