...
[![Open in Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/profrodai/sovereign-agent/blob/main/book/exercises/ch11/profrod-sovereign-agent-ch11-a-durable-order-evidence-exercise.ipynb) Runs on Google Colab as it ships today, or on any local Python 3.12+ kernel.
...
def grade_transition(candidate):
    with tempfile.TemporaryDirectory(prefix="ch09-transition-visible-") as directory:
        ...
        try:
            ...
            money = rows[-1]["reserved_cents"], rows[-1]["spent_cents"]
...
The local `SENDING` commit happens first. The supplier then commits one remote receipt and closes the connection. Your `UNKNOWN` transition retains 1,500 cents. Reconciliation reads the supplier's existing receipt and records it without another order.
    ...
    with tempfile.TemporaryDirectory(prefix="ch09-connected-") as directory:
        ...
        with independent_supplier(root) as (supplier, supplier_path):
            ...
            try:
                ...
                assert (after_loss["reserved_cents"], after_loss["spent_cents"]) == (1500, 0)
                assert final["status"] == "CONFIRMED"
                assert (final["reserved_cents"], final["spent_cents"]) == (0, 1500)
...
from sovereign_agent.assistant_orders import SpendingPolicy, execute
...
llm_report_path = COURSE_WORK / "ch11-a-llm-lab-report-v1.json"
