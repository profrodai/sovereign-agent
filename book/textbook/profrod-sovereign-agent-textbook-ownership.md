...
Chapter 1 is a direct model-request example with an offline response fixture. Chapter 2 constructs tool arguments, handlers and dispatch in `learner/profrod_sovereign_agent_ch02_pydantic_shop_tools_learner.py`, reusing Chapter 1's shop data. Chapter 3 constructs the model/tool loop and parsing in `learner/profrod_sovereign_agent_ch03_agent_loop_learner.py`. Its HTTP path imports the supplied bounded transport from `sovereign_agent.http_transport`. The saved learner files are completed comparison implementations; use your own checkout to construct and change them.
