...
**Status: DRAFT.** Read the [textbook guide](../README.md) for setup and supplied-code boundaries. Practice in [Exercise Book 2](../../exercises/ch02/README.md); consult [Solutions 2](../../solutions/ch02/README.md) after attempting the work.
...
For dedicated practice, use [Unit A](../../exercises/ch02/unit-a.md) and [Unit B](../../exercises/ch02/unit-b.md), each with its own ninety-minute plan and matching notebook.
...
We continue with the same three products from Chapter 1. The standalone checkpoint keeps its stock in memory so that the dispatch mechanism can be inspected without first teaching database transactions. Chapter 5 will replace this storage boundary with persistent records. For now, the complete tool layer uses only Python and Pydantic.
...
SHOP = runpy.run_path("book/textbook/checkpoints/ch01.py")["SHOP"]
...
The standalone checkpoint collects the fixture and shop functions in `book/textbook/checkpoints/ch02.py`. It imports the small request and dispatcher classes whose implementation you have just built from the cumulative source tree. These are this book's own components, and their complete execution path appears above. There is no hidden agent framework selecting or running tools.
...
```bash
uv run python book/textbook/checkpoints/ch02.py
...
The tools can now answer useful questions, but someone still has to choose the sequence of calls. In [Chapter 3](../ch03/README.md), you will connect model responses, tool requests, and observations in a bounded loop. The model will select requests while the Python runtime retains control of what can execute and when the turn must stop.
