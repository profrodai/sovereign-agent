...
**Status: DRAFT.** Read the [textbook guide](../README.md) for setup and supplied-code boundaries. Practice in [Exercise Book 8](../../exercises/ch08/README.md); consult [Solutions 8](../../solutions/ch08/README.md) after attempting the work.

The planned Chapter 7 gives durable work and report creation their own construction lesson. This draft retains its existing queue, cursor and outbox explanations and supplied reference implementation until that learner handoff is authored. Read [Chapter 7](../ch07/README.md) for the intended separation and acceptance cases.
...
In this chapter you will build one Telegram adapter around durable intake, stable session identity and an explicit outbound delivery state. You will keep the local bot interface available for offline exercises. The same loop and active opening skill from [Chapter 6](../ch06/README.md) will perform the work. Consequential purchasing remains outside this chapter's dispatcher.
...
The reserved control-command allowance is for later approval, revocation and cancellation work. This chapter's dispatcher exposes stock and draft tools only; a command-looking message cannot create purchasing authority. Chapter 10 will connect explicit command interpretation to exact proposals. Here the important admission property is that every persisted request has its original identity and payload, including an explicit rejection when it cannot enter ordinary work.
...
Two requests in the same private session should not concurrently assemble context from the same unfinished conversation. The existing claim boundary serializes that session. Different sessions can be handled separately when the worker design supports it. Chapter 12 will construct ownership generations and crash replacement in detail; here we can already demonstrate the visible claim behavior using two database connections.
...
seed_lucy(db)
source = Path("book/textbook/skills/opening-check-v1.toml")
...
draft_checker = runpy.run_path("book/textbook/checkpoints/ch03.py")["draft_evidence"]
...
checkpoint = runpy.run_path("book/textbook/checkpoints/ch08.py")
with patch.object(sys, "argv", ["ch08.py"]):
...
```bash
uv run python book/textbook/checkpoints/ch08.py --telegram --root /tmp/lucy-phone-test
```
