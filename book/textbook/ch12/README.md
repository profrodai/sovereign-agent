...
**Status: DRAFT.** Read the [textbook guide](../README.md) for setup and supplied-code boundaries. Practice in [Exercise Book 12](../../exercises/ch12/README.md); consult [Solutions 12](../../solutions/ch12/README.md) after attempting the work.
...
The research-role branch anticipates Chapter 17. A delegated worker also needs a live assignment deadline and an uncancelled parent. It uses the same ownership check and adds its narrower contract. We keep that branch visible because the cumulative runtime has one current-holder function; the ordinary shop examples do not grant a research role or rely on delegation.
...
```bash
uv run python book/textbook/checkpoints/ch12.py
uv run python book/textbook/checkpoints/ch12.py --evidence
```
...
checkpoint = runpy.run_path("book/textbook/checkpoints/ch12.py")
...
The new checkpoint kills its worker before the first supplier transmission. The repository also has an actual `agent serve` test that waits until the supplier has committed an order, then kills the agent before its local receipt is recorded. That case combines this chapter's replacement ownership with Chapter 11's ambiguous effect. The replacement must reconcile the original operation before processing unrelated later work.
