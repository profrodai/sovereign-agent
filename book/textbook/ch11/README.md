...
**Status: DRAFT.** Read the [textbook guide](../README.md) for setup and supplied-code boundaries. Practice in [Exercise Book 11](../../exercises/ch11/README.md); consult [Solutions 11](../../solutions/ch11/README.md) after attempting the work.

For practical work, use the [Chapter 11 successor exercises](../../exercises/ch11/README.md): build the durable transition, reproduce a real duplicate-purchase defect, repair the cumulative order path, and transfer reconciliation to a second supplier shape.
...
checkpoint = runpy.run_path("book/textbook/checkpoints/ch11.py")
checkpoint["main"]()
```
...
```bash
uv run python book/textbook/checkpoints/ch11.py
uv run pytest tests/test_assistant_durability.py
uv run python scripts/verify_book_assets_v1.py --textbook
```
...
The next question concerns the worker carrying that intent. If its process dies and another starts, which one may continue? [Chapter 12](../ch12/README.md) separates durable work from its current owner and rejects newly authorized writes from stale workers.
