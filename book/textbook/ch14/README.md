...
**Status: DRAFT.** Read the [textbook guide](../README.md) for setup and supplied-code boundaries. Practice in [Exercise Book 14](../../exercises/ch14/README.md); consult [Solutions 14](../../solutions/ch14/README.md) after attempting the work.
...
This particular worker has no supplier tool enabled. When a later scenario supplies one, the Chapter 10 approval and [Chapter 11 effect checks](../ch11/README.md) remain necessary. A read-only scenario with zero orders does not by itself prove an enabled purchasing scenario safe. Likewise, a container's inability to contact the supplier does not excuse a host tool that sends orders without checking authority.
