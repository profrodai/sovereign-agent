...
**Status: DRAFT.** Read the [textbook guide](../profrod-sovereign-agent-textbook-start-here.md) for setup and supplied-code boundaries. Practice in [Exercise Book 14](../../exercises/ch14/profrod-sovereign-agent-ch14-tool-isolation-exercise-guide.md); consult [Solutions 14](../../solutions/ch14/profrod-sovereign-agent-ch14-tool-isolation-solutions-guide.md) after attempting the work.

Chapter 13 reserves dedicated space for MCP construction. Its full lesson is still planned, so this draft retains the existing MCP material alongside containment. The future split must preserve these explanations while making the difference between protocol, application permission and OS enforcement explicit. See [the Chapter 13 brief](../ch13/profrod-sovereign-agent-ch13-mcp-tools-chapter.md).
...
The model's final text explicitly tells us to inspect the result, but that wording is not part of the enforcement proof. Replacing it with “the purchase succeeded” would not create an order. It would create a false answer that our outcome evaluation should reject. Chapter 15 separates these two dimensions: preventing unauthorized effects and producing useful, grounded answers.

This particular worker has no supplier tool enabled. When a later scenario supplies one, the Chapter 10 approval and [Chapter 11 effect checks](../ch11/profrod-sovereign-agent-ch11-ambiguous-supplier-order-chapter.md) remain necessary. A read-only scenario with zero orders does not by itself prove an enabled purchasing scenario safe. Likewise, a container's inability to contact the supplier does not excuse a host tool that sends orders without checking authority.
